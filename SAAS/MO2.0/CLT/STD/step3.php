...
$(function() {
    $('#check_list_veiculo_p').change(function(e) {

      check_veiculos = document.getElementById('text_proprios').value;
      retorno = check_veiculos.split(";");
      troca_virgula = check_veiculos.replace(';', ",");

      if ($(this).is(':checked')) {

        for (i = 0; i < retorno.length; i++){
        $('tr input[type=checkbox][value=' + "'" + retorno[i] + "'"+ ']').prop("checked", true);

        
 if(i==retorno.length -1){
  document.getElementById('text_proprios_v').value += "'" + retorno[i] + "'";
 } else {
  document.getElementById('text_proprios_v').value += "'" + retorno[i] + "',";
 }
        ...
        for (i = 0; i < retorno.length; i++){
        $('tr input[type=checkbox][value=' + "'" + retorno[i] + "'"+ ']').prop("checked", false);
        document.getElementById('text_proprios_v').value = "";
      }
...
$(function() {
    $('#check_list_veiculo_t').change(function(e) {

      check_veiculos = document.getElementById('text_trans').value;
 ...
 if(i==retorno.length -1){
  document.getElementById('text_trans_v').value += "'" + retorno[i] + "'";
 } else {
  document.getElementById('text_trans_v').value += "'" + retorno[i] + "',";
 }
        ...
        document.getElementById('text_trans_v').value = "";
      }
...
var styles = [
  ...
  $(document).ready(function() {
    ...
    var options_x = document.getElementById("lang0pt").options;
   
for (var i = 0; i < options_x.length; i++) {

  alert("ok");

  if (options_x[i].text == "AGE2021") {
    options_x[i].selected = true;
    break;
  }
}
...
if(!empty($_POST['text_proprios_v'])) {
  $campo_text_p = $_POST['text_proprios_v'];
  $string_text_p = $campo_text_p;
  $completa_text_p = " AND usuario.login in($string_text_p) ";

}
if(!empty($_POST['text_trans_v'])) {
  $campo_text_x = $_POST['text_trans_v'];
  $string_text_x = $campo_text_x;
  $completa_text_x = " AND usuario.login in($string_text_x) ";
...
//TODSO FILTROS MYSQL
$query = "select * from rotas where site=1 "  . $completa0 . $completa1 . $completa3;
...
//FILTRO CODIGO PEDIDO
    
if($string4!=""){


    $query = "select * from rotas where codigo_pedido in($string4)";

    $query_pizza1 = "select id from rotas where status=1 AND codigo_pedido in($string4)";													
    $rs_pizza1 = mysql_query($query_pizza1);
    $num_rows_pizza1 = mysql_num_rows($rs_pizza1);
   
    
    $query_pizza2 = "select id from rotas where status=2 AND codigo_pedido in($string4)";															
    $rs_pizza2 = mysql_query($query_pizza2);
    $num_rows_pizza2 = mysql_num_rows($rs_pizza2);
  
    
    
    $query_pizza3 = "select id from rotas where status=0 AND codigo_pedido in($string4)";															
    $rs_pizza3 = mysql_query($query_pizza3);
    $num_rows_pizza3 = mysql_num_rows($rs_pizza3);
  
    
    $query_pizza4 = "select id from rotas where status=3 AND codigo_pedido in($string4)";														
    $rs_pizza4 = mysql_query($query_pizza4);
    $num_rows_pizza4 = mysql_num_rows($rs_pizza4);
  ...
  $query = "select * from rotas where statusrota=10";
...
?> 	
    ...
    <tr>
     	   <td align="left" valign="middle"><font size="3">PAINEL DE TV</font></td>
...
<form name="search_form1" method="POST" action="step3.php">
    ...
    <hr size = 1 width = '200px' color="#DCDCDC">
      ...
      <tr>
          <td nowrap="nowrap" height="30px" bgcolor="#ECECEC" style="padding-left: 10px;">
            <i class="material-icons" style="font-size:22px; vertical-align:bottom">&#xe853;</i><font size="2"> Pedidos</font>
          </td>	
      </tr>
        <tr >
...
<form name="search_form" method="POST" action="step3.php">
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" align="left" style="padding-top: 25px; padding-bottom: 8px; width:360px" >
        ...
        $query_veiculos = "Select * from usuario where coordenador='transportadora' and nivel=2";
        $rs_v = mysql_query($query_veiculos);	

        while($row_v = mysql_fetch_array($rs_v)){
          array_push($stack, $row_v['login']);
          array_push($syz1, "'" . $row_v['login'] . "'");
        }
        $comma_separated = implode(";", $stack);
        $comma_separated_x = implode(",", $syz1);
        /////////////////////////////////////////////////////////////////////////////////////////


        $query_veiculos1 = "Select * from usuario where coordenador!='transportadora' and nivel=2";
        $rs_v1 = mysql_query($query_veiculos1);	

        while($row_v1 = mysql_fetch_array($rs_v1)){
          array_push($stack1, $row_v1['login']);
          array_push($syz, "'" . $row_v1['login'] . "'");
          
        }
        $comma_separated1 = implode(";", $stack1);
        $comma_separated_v = implode(",", $syz);

        ?>
        <table border="0" cellpadding="0" cellspacing="5" width="340px">
			
      <tr>
          <td nowrap="nowrap" height="30px" bgcolor="#ECECEC" style="padding-left: 10px;" colspan="2">
            <i class="material-icons" style="font-size:22px; vertical-align:bottom">directions_car</i><font size="2"> Veículos</font>
          </td>	
      </tr>
      <tr>
          <td nowrap="nowrap" height="30px" bgcolor="#FFF" style="padding-left: 10px; width:50px">
           
          <input type="checkbox" class="marcar" name="check_list_veiculo_p" id="check_list_veiculo_p" value='proprio'>&nbsp;&nbsp;Próprios
          <input type="text" name="text_proprios" id="text_proprios" value="<?php echo $comma_separated1; ?>" hidden>
          <input type="text" name="text_proprios_v" id="text_proprios_v" value="" hidden>
          </td>	
          <td nowrap="nowrap" height="30px" bgcolor="#FFF" style="padding-left: 10px;">
           
           <input type="checkbox" class="marcar" name="check_list_veiculo_t" id="check_list_veiculo_t" value='<?php echo $comma_separated; ?>'>&nbsp;&nbsp;Transportadoras
           <input type="text" name="text_trans" id="text_trans" value="<?php echo $comma_separated; ?>" hidden>
           <input type="text" name="text_trans_v" id="text_trans_v" value="" hidden >
           </td>	
      </tr>
  </table>
  <br> 
...
$query_usuarios = "SELECT * FROM rastro r1 INNER JOIN usuario ON usuario.login = r1.login WHERE r1.id = (SELECT max(r2.id) FROM rastro r2 WHERE r2.login = r1.login) AND usuario.nivel>0" . $completa_text_p . $completa_text_x;
...
<div id="total_mapa"></div>
  
 </div>
</div>
<div id='embaixo'>
  
<table border="0" style="height: 40px; color:#CCCCCC;"  cellpadding="0" cellspacing="0" width="100%">
<tr>
...
if($string0=="" AND $string1=="" AND $string3=="") {
  ...
  if($string4!=""){
    $completa0 = " AND codigo_pedido in($string4)";
  } else {
