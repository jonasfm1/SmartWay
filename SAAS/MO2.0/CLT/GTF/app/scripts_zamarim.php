...
$check2 = mysqli_query($con1,$query_where1) or die("Erro 1:" .mysqli_error ($con1));
...
//echo $userlogin;
$con=mysql_connect("$host_x", "teste", "7s9KfCGxN6zEvBYT")or die("Não conectado ao servidor!"); 
...
$sql = "SELECT rota, statusrota FROM rotas WHERE login='$userlogin' AND statusrota>0 GROUP BY rota ORDER BY statusrota, lista, rota"; 
$result = mysql_query($sql);
$json = array();
...
$f = fopen($concatena_txt, 'a');
	...
	fwrite($f, strtolower($_GET['usuario']).';');
    fwrite($f, $_GET['locx'].";");
	fwrite($f, $_GET['locy'].";");
    fwrite($f, $_GET['dtEt']."\r\n");
    fclose($f);
...
case 'realizado_entrada':
       ...
       list ( $log, $banco) = explode('@',$_GET['usuario']);
	   $banco = "mo_".$banco;
       $concatena_txt = $banco . "/REALIZADO.txt";
      // $f = fopen($concatena_txt, 'a');  
	    
	   list ( $vis, $cli, $cid) = explode('@',$_GET['cliente']);
	   $oco="Chegada";
	   $dth=$_GET['dataentrada'];
	   list ( $y, $x) = explode(',', $_GET['local']);
	   $sta=1; 
...
case 'realizado_ocorrencia':
      ...
      // $f = fopen($concatena_txt, 'a');
	   list ( $vis, $cli, $cid) = explode('@',$_GET['cliente']);
	   list ( $oco, $sta) = explode('@',$_GET['ocorrencia']);   
...
case 'enviar_imagem':
	...
	if(preg_match('/^(save-form){1}$/', $_GET['method'])){
		
		$imgName = 'img-'.time().'.'.$_GET['img-mime'];
		
		$f = fopen('DATA.txt', 'a');
		fwrite($f, " Name: ".$_GET['name']."\n");
		fwrite($f, " Nome: ".$_GET['img-mime']."\n");
		fwrite($f, " Image: ".$imgName."\r\n");
		fclose($f);
		
		$binary = base64_decode($_GET['img-image']);
