...
<table width="100%" border="0" cellpadding="0" cellspacing="0" style="background-color:#579bd3; font-size: 11px; color: white" >
    <tr>
		<td align="left" valign="center" colspan="3" height="50px"><i class="material-icons" style="font-size:22px; vertical-align:bottom;padding-left: 15px;">people_outline</i><strong><font size="2px">&nbsp;ALTERAR LOGIN</font></strong>  
      </td>
    </tr>
</table>
</div>
...
//echo $procurando;
 if($procurando!=""){
  ...
  $query1 = "select * from usuario where nivel=2 and login like '%$procurando%' ORDER BY login ASC";													
...
<form action="step2_sm_l.php" method="GET" name="procura" id="procura">
<?php
		 for ($i = 0; $i < $conta_lista; $i++) {
			?>
...
<form action="scripts.php?acao=altera_login_site" method="POST" name="envia" id="envia" onsubmit="return verificar();">		  
...
<label class="container"><i class="material-icons" style="font-size:24px; vertical-align:middle; color: gray; align-content: center; padding-right: 3px; padding-left: 10px">account_circle</i><span style="vertical-align:inherit"><?php echo $row1["login"];?></span>
  <input type="radio" value="<?php echo $row1["login"];?>" name="RadioGroup1"  id="RadioGroup1">
