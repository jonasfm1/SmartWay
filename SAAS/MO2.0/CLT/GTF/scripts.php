...
case 'excluir_registro':
   ...
   // echo $campo;
    ...
    ///////////// ADICIONA TEMPO VISITA APP ////////////////////////////////

case 'adiciona_tempo_visita_app':
  ...
  //  $data_dia = date('Y-m-d H:i:s');
        ...
        for ($i = 0; $i <= $conta_lista; $i++) {
            $query2 = "UPDATE rotas SET tempo_est='$horario' where id='$campo[$i]'";
            $rs2 = mysql_query($query2);
   ...
   // echo $minuto;
    ...
    <table width="100%" height="100%" style="background-color:#FFF; vertical-align: middle; text-align: center">
        ...
        <td><img src="imgs/ok.PNG"/>
            <br>
        <strong>TEMPO DE VISITA INSERIDA COM SUCESSO!</strong>
            </td>
        </tr>
    ...
    ////////////// ADICIONA TEMPO VISITA APP////////////////////////////////			
     ...
     ///////////// ADICIONA SUB NATUREZA APP ////////////////////////////////

case 'adiciona_sub_app':

    //$id= $_POST['quais'];
        $numero_oco = $_POST['sub'];
        $campo = $_POST['check_list'];
        $conta_lista = count($campo);
 
    
        for ($i = 0; $i <= $conta_lista; $i++) {
            $query2 = "UPDATE rotas SET subnatureza='$numero_oco', quem='$user' where id='$campo[$i]'";
            $rs2 = mysql_query($query2);
    ...
    <table width="100%" height="100%" style="background-color:#FFF; vertical-align: middle; text-align: center">
        ...
        <td><img src="imgs/ok.PNG"/>
            <br>
        <strong>SUB NATUREZA ADICIOANDA COM SUCESSO!</strong>
            </td>
        </tr>
...
//echo $login . "<br>";
        ...
        if ($pega_slt == 'slt_gsp1' or $pega_slt == 'slt_gsp2' or $pega_slt == 'slt_i1' or $pega_slt == 'slt_i2') {
            ...
            $query2 = "INSERT INTO rotas(login, rota, sequencia, x, y, idcliente, nome, endereco, cidade, classificacao, tempo, obs, raio, status, statusrota, lista, codigo_pedido) VALUES('$login', '$rota', '$ordem', '$x', '$y', '$codigo_cliente', '$nome_cliente', '$endereco', '$cidade', '$codigo_pedido', '$tempo_servico', '$obs_pedido', '$raio', '$status', 1, '$lista', '$codigo_pedido')";
            $rs2 = mysql_query($query2);
...
case 'atualiza_tempo':
    ...
    $duracao = '01:00:00';
    $v = explode(':', $tempo_est);
    $soma_ok = date('Y-m-d H:i:s', strtotime("{$data} + {$v[0]} hours {$v[1]} minutes {$v[2]} seconds"));


    $query2 = "UPDATE rotas SET dth_chegada='$data', dth_saida='$soma_ok' WHERE id='$id'";
    $rs2 = mysql_query($query2);
...
case 'atualiza_endereco':
    ...
    $query = "UPDATE rotas SET y='$new_lat', x='$new_lgn' where id='$codigo'";															
    $rs = mysql_query($query);
 ...
 // GRAVA NOVO CLIENTE NO DB_GERAL ROTAS
 
 $con1 = mysql_connect("127.0.0.1", "root", "HtMQZhAwCNzeaAfT") or die ("Sem conexão com o servidor");
 $select1 = mysql_select_db($new) or die("Sem acesso ao DB, Entre em contato com o Administrador!");
 
 $query_verifica = mysql_query("SELECT codigo from db_geral where codigo_cliente='$reg'");
 $num_rows = mysql_num_rows($query_verifica);

if ($num_rows > 0){
    $query20 =  "UPDATE db_geral SET y='$new_lat', x='$new_lgn' where codigo_cliente='$reg'";															
    $rs20 = mysql_query($query_update);

} else {


    $query20 = "INSERT INTO db_geral(codigo_cliente, nome, endereco, cidade, estado, cep, latitude_cad, longitude_cad, endereco_cad, bairro, confianca_cad) values ('$reg', '$nome', '$endereco', '$cidade', '$uf', '$cep', '$new_lat', '$new_lgn', '$endereco', '$bairro', 101)";
    $rs20 = mysql_query($query20);
}
