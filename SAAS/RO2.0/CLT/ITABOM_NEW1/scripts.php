...
case 'adiciona_transp':
	$nome = $_POST["name"];
	$sigla = $_POST["sigla"];

	$query_cadastrar = "INSERT into transportadora(nome, sigla) values ('$nome', '$sigla')";
	mysql_query($query_cadastrar) or die(mysql_error());
	?>

<SCRIPT LANGUAGE="JavaScript" TYPE="text/javascript">
	  window.location.href = "transp_control.php";
...
case 'edita_transp':
	...
	$query_cadastrar1 = mysql_query("UPDATE transportadora SET nome='$nome', sigla='$sigla' where nome='$id'");
	$query_cadastrar2 = mysql_query("UPDATE transportadora_itabom SET transportadora='$nome' where transportadora='$id'");

	$query_procura1 =  mysql_query("UPDATE controle_regiao SET transportadoras= REPLACE(transportadoras, '$id', '$nome' ) where transportadoras like '%$id%'");
	$query_procura1 =  mysql_query("UPDATE clientes SET transportadoras= REPLACE(transportadoras, '$id', '$nome' ) where transportadoras like '%$id%'");
	$query_procura1 =  mysql_query("UPDATE veiculos SET transportadora='$nome' where transportadora ='$id'");
...
case 'edita_regiao':
	$nome = $_POST["name"];
	//$nome_old = $_POST["name_old"];
	$dia = $_POST["check_list"];
	$transp = $_POST["check_list1"];
	$veiculo = $_POST["check_list2"];
	//echo $veiculo[0];
	
	$conta_dia = count($dia);
	$conta_transp = count($transp);
	$conta_veiculo = count($veiculo);
	
	for ($i = 0; $i < $conta_dia; $i++) {
	  $guarda_dia .= $dia[$i] . ', ';
	  if($dia[$i]=="seg"){
		$seg=1;
	  }
	...
	for ($i = 0; $i < $conta_transp; $i++) {
		$guarda_transp .= $transp[$i] . ',';
	  }
	  for ($i = 0; $i < $conta_veiculo; $i++) {
		$guarda_veiculo .= $veiculo[$i] . ',';
	  }
	//echo $guarda_veiculo;

	$query_cadastrar = "UPDATE controle_regiao SET regiao='$nome', transportadoras='$guarda_transp', veiculos='$guarda_veiculo', seg='$seg', ter='$ter', qua='$qua', qui='$qui', sex='$sex', sab='$sab' where regiao='$nome'";
	mysql_query($query_cadastrar) or die(mysql_error());

	?>

<SCRIPT LANGUAGE="JavaScript" TYPE="text/javascript">
	  window.location.href = "regioes_control.php";
...
case 'cadastra_regiao':
	...
	//echo $guarda_veiculo;
	 $query_cadastrar = "INSERT into controle_regiao(regiao, transportadoras, veiculos, seg, ter, qua, qui, sex, sab) values ('$nome', '$guarda_transp', '$guarda_veiculo', '$seg', '$ter', '$qua', '$qui', '$sex', '$sab')";
	 mysql_query($query_cadastrar) or die(mysql_error());
...
<table width="100%" height="100%" style="background-color:#FFF; vertical-align: middle; text-align: center">
	...
	<td><img src="imgs/ok.PNG"/>
		<br>
	<strong>REGIÃO CADASTRADA COM SUCESSO!</strong>	
		</td>
	</tr>
...
if($endereco==""){
	?>
		<SCRIPT LANGUAGE="JavaScript" TYPE="text/javascript"> 
		alert("Cliente <?php echo $codigo_cliente; ?> sem endereço.\nPor favor, complete o cadastro de endereço e localize o ponto na Geocodificação Manual.");
		window.history.go(-1);
	...
	<?php
		...
		$sql = mysql_query("UPDATE db_geral SET codigo_cliente='$codigo_cliente', nome='$nome', endereco='$endereco', cidade='$cidade', estado='$estado', endereco_cad='$endereco_cad', latitude_cad='$latitude_cad', longitude_cad='$longitude_cad', confianca_cad='101', bairro='$bairro', regiao_itabom='$regiao' WHERE codigo_cliente = '$codigo_cliente'");
		
		} else {
			
		// faz inserção
		$sql = mysql_query("INSERT INTO db_geral(codigo_cliente, nome, endereco, cidade, estado, cep, endereco_cad, latitude_cad, longitude_cad, confianca_cad, bairro, regiao_itabom) VALUES('$codigo_cliente', '$nome', '$endereco', '$cidade', '$estado', '$cep', '$endereco_cad', '$latitude_cad', '$longitude_cad', '101', '$bairro', '$regiao')");
		}
...
//while($row = mysql_fetch_row($rs)){	
	...
	if($rede=='1'){
		$premium= 'FFA500';	
		$query2 = "UPDATE db_geral SET nome='$nome_cliente', endereco='$endereco_cliente', cidade='$cidade_cliente', estado='$uf_cliente', cep='$cep_cliente', bairro='$bairro_cliente', premium='$premium', regiao_itabom='$regiao' WHERE codigo_cliente='$cod_cliente'";
		$rs2= mysql_query($query2);
		}else{
			$query2 = "UPDATE db_geral SET nome='$nome_cliente', endereco='$endereco_cliente', cidade='$cidade_cliente', estado='$uf_cliente', cep='$cep_cliente', bairro='$bairro_cliente', regiao_itabom='$regiao' WHERE codigo_cliente='$cod_cliente'";
			$rs2= mysql_query($query2);
		}
...
while($row = mysql_fetch_array($rs)){	
	...
	$query2 = "UPDATE clientes SET nome='$nome_cliente', endereco='$endereco_cliente', bairro='$bairro_cliente', cidade='$cidade_cliente', estado='$uf_cliente', cep='$cep_cliente', premium='$premium_novo', data_agendado='$data_formatada', obs_agendado='$obs_agendada', regiao_itabom='$regiao' WHERE codigo_pedido='$cod_pedido'";
	$rs2= mysql_query($query2);

	
	$rsx = mysql_query("UPDATE db_geral SET nome='$nome_cliente', endereco='$endereco_cliente', bairro='$bairro_cliente', cidade='$cidade_cliente', estado='$uf_cliente', cep='$cep_cliente', regiao_itabom='$regiao' where codigo_cliente='$cod_cliente'");

  
}


$query_regiao = "select * from regiao_itabom WHERE regiao='$regiao'";													
$rs_regiao = mysql_query($query_regiao);
$total = mysql_fetch_assoc($rs_regiao);

if($total>0){

} else {
	$query_vai = "INSERT INTO regiao_itabom(regiao) VALUES ('$regiao')";
	$rs_vai= mysql_query($query_vai);

}

$query_verifica_regiao ="SELECT * from controle_regiao where regiao='$regiao'";
$rs_verifica_regiao = mysql_query($query_verifica_regiao);

$dados = mysql_fetch_array($rs_verifica_regiao);
...
$junta_transp .= $dados['transportadoras'] . ", ";
$junta_veiculos .= $dados['veiculos'] . ", ";



$query_atualiza = "UPDATE clientes SET seg='$seg', ter='$ter', qua='$qua', qui='$qui', sex='$sex', sab='$sab', transportadoras='$junta_transp', quais_veiculos='$junta_veiculos' WHERE codigo_cliente='$cod_cliente'";
$rs_atualiza= mysql_query($query_atualiza);
...
$query = "select * from clientes WHERE codigo_cliente='$cod_cliente'";															
$rs = mysql_query($query);
...
while($row = mysql_fetch_array($rs)){	
 ...
 $query2 = "UPDATE clientes SET nome='$nome_cliente', endereco='$endereco_cliente', bairro='$bairro_cliente', cidade='$cidade_cliente', estado='$uf_cliente', cep='$cep_cliente', regiao_itabom='$regiao' WHERE codigo_cliente='$cod_cliente'";
  $rs2= mysql_query($query2);
...
$query="SELECT nome from veiculos where nome='$veiculo'";
$rs = mysql_query($query);
...
if($total>0){
	?>
	<SCRIPT language="JavaScript">
	alert("Esse PLACA já foi cadastrado!");
	window.location.href="cad_veiculos.php";
...
if($ativo==on){
	$query2 = "INSERT INTO veiculos(nome, peso, volume, valor, cor, tipo, type_icon, ativo, custo, setor, transportadora, motorista, eixos) VALUES('$veiculo', '$peso', '$volume', '$valor', '$cor', '$tipo', '$type_icon', 'yes', '$custo', '$setor', '$transportadora', '$motorista', '$eixos')";

} else {
	
	$query2 = "INSERT INTO veiculos(nome, peso, volume, valor, cor, tipo, type_icon, custo, setor, transportadora, motorista, eixos) VALUES('$veiculo', '$peso', '$volume', '$valor', '$cor', '$tipo', '$type_icon', '$custo', '$setor', '$transportadora', '$motorista', '$eixos')";
...
//if($ativo==on){
$query2 = "UPDATE veiculos SET nome='$veiculo', peso='$peso', volume='$volume', valor='$valor', cor='$cor', type_icon='$type_icon', tipo='$tipo', setor='$custo', custo='$custo1', transportadora='$transportadora', motorista='$motorista', regiao='$regiao', eixos='$eixos' where id='$id_quem'";
//} else {
...
/*	
$select_nome = "SELECT nome_rota from rotas where veiculo='$i_id_veiculo[$y]'";
$dados_ok = mysql_fetch_array($select_nome);
$somadata_rota = $dados_ok['nome_rota'];

*/
	$transp_query = "select transportadora, setor, prioridade from veiculos where id='$i_id_veiculo[$y]'";
	$rs_transp = mysql_query($transp_query);
	...
	if($transp==''){
		...
		$query3 = "UPDATE rotas SET veiculo='$i_id_veiculo[$y]', prioridade=$prio, transportadora='$transp', ordem='$i_ordem[$y]', distancia='$i_distancia[$y]', tempo='$i_tempo[$y]', tempo_servico='$data_hoje', data_entrega='$data_hoje', redespacho='$redespacho' WHERE id = '$i_id_cliente[$y]'";
...
case 'grava_lat_lgn_veiculo':
	...
	$check_transbordo = $_POST["trans_ok"];
	$loc_inicio_trans= $_POST["inicio_transbordo"];
	$loc_final_trans= $_POST["final_transbordo"];
...
//	echo $check_transbordo;
//	echo $loc_inicio_trans;
//	echo $loc_final_trans;

foreach($check as $indice => $valor)
{
   //var_dump($indice, $valor);

   if ($check_transbordo=='on'){
	$query = "UPDATE veiculos SET local_inicio='$loc_inicio_trans', local_final='$loc_final_trans' where id='$valor'";
//print("entrou");
   } else {
//	print(" n entrou");
...
case 'limpa_filtro_step3':
	...
	include ('session.php'); 
	unset($_SESSION['regiao_itabom']);
	unset($_SESSION['dia']);
	unset($_SESSION["classificacao"]);
	unset($_SESSION["transportadora"]);
	unset($_SESSION["veiculos"]);
