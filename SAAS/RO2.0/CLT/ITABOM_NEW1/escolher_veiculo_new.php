...
<th nowrap="nowrap" headers="cod" bgcolor="#00000" style="color:#000000;" class="drag-enable" title="MOTORISTA" ><div align="center" >MOTORISTA</div></th>
...
if($string0==""){
	...
	for ($i = 0; $i < $conta_campos; $i++) {
 			if($conta_campos==1){
				...
				$completa1 = " AND nome=" .   "'$campo0[$i]'";
				//$completa4 .= ")";
 			} else {

				if($i == ($conta_campos-1)){

					$completa1 .=  "nome=" . "'$campo0[$i]'";
				} else {
					$completa1 .=  "nome=" . "'$campo0[$i]' OR ";
...
if($string0==""){
	...
	for ($i = 0; $i < $conta_campos; $i++) {
 			if($conta_campos==1){
				...
				$completa4 = " AND transportadora='" .$campo0[$i] . "' ";
				//$completa4 .= ")";
 			} else {

				if($i == ($conta_campos-1)){

					$completa4 .=  " transportadora='" . $campo0[$i]. "' ";
				} else {
					$completa4 .=  " transportadora='" . $campo0[$i] . "' OR ";
...
if($string0==""){
	...
	for ($i = 0; $i < $conta_campos; $i++) {
 			if($conta_campos==1){
				...
				$completa2 = " AND regiao_itabom" .   " LIKE ('%$campo0[$i]%')";
				//$completa4 .= ")";
 			} else {

				if($i == ($conta_campos-1)){

					$completa2 .=  "regiao_itabom" . " LIKE ('%$campo0[$i]%')";
				} else {
					$completa2 .=  "regiao_itabom" . " LIKE ('%$campo0[$i]%') OR ";
...
//echo $completa2;
if($completa4!='' && $completa1!=''){



	$query_veiuclos = "select motorista, peso_total,volume_total,valor_total,peso,volume,valor,cor,tipo,nome,transportadora,id, ocupado from veiculos where ativo='yes' AND roteirizado!='sim' " . $completa1 . " order by transportadora asc, nome asc";	


} else {

	
$query_veiuclos = "select motorista, peso_total,volume_total,valor_total,peso,volume,valor,cor,tipo,nome,transportadora,id, ocupado from veiculos where ativo='yes' AND roteirizado!='sim' " . $completa0 . $completa1 . $completa2.  $completa3 .  $completa4 . " order by transportadora asc, nome asc";	
...
<td  style="padding-left: 5px;" nowrap="nowrap"><?php echo $row_veiculo["motorista"] ?></td>
