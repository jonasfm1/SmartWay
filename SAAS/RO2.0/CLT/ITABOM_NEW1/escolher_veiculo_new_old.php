...
<?php 
		$query_veiuclos = "select * from veiculos where ativo='yes' AND roteirizado!='sim' order by transportadora asc, nome asc";															
		$rs_veiculo = mysql_query($query_veiuclos);

		while($row_veiculo = mysql_fetch_array($rs_veiculo)){

			$peso_recuperado = $row["peso_total"];
			$volume_recuperado = $row["volume_total"];
		
			$valor_recuperado = $row["valor_total"];
