...
<table width="750px" border="1" cellspacing="2" cellpadding="2" class="bordasimples">
...
if ($transp==''){
  $query1 = "select * from rotas where nome_rota!='' group by nome_rota order by prioridade ASC";		
} else {
  $query1 = "select * from rotas where nome_rota!='' and transportadora='$transp' group by nome_rota order by prioridade ASC";		
...
<table width="750px" border="1" cellspacing="2" cellpadding="2" class="bordasimples">
  <tbody>
    <tr>
     <td height="40px" align="right" width=650px" style="font-weight:bold;background-color: #2867b2;" colspan="5"><font color="#FFF" size="3">&nbsp;&nbsp;<input type="checkbox" value="<?=$nome_rota?>" class="marcar" name="check_list[]" id="check_list"  />&nbsp;&nbsp;<?php echo $caminhao; ?></font></td>
...
if($data['total']<=25){
?>
 <input type="submit" name="submit" id="submit" value="ORDENAR" onclick="javascript:acao1('<?php echo $nome_rota ?>', '<?php echo $caminhao ?>');" />
...
<input type="submit" name="submit" id="submit" value="ORDENAR" onclick="javascript:acao2('<?php echo $nome_rota ?>', '<?php echo $caminhao ?>');" />  
<?php
}
 ?>
    </td>
  ...
  <tr>
    <td height="20px" align="left" style="font-weight:bold;"><font color="#000000" size="1">VISITAS: <?php echo $conta_i; ?></td>
     ...
     <td height="20px" align="left" style="font-weight:bold;"><font color="#000000" size="1"></td>
    </tr>
  </tbody>
</table>
...
<tr bgcolor="#ECECEC" height="20" font color="#000000" style="font-weight: bold;">  
<td width="49"> 
<div>ROTA</div>
</td> 
<td width="49"> 
...
while($row_todos = mysql_fetch_array($rs_todos)){
  ...
  $queme_pedagio = $row_todos["qto_pedagio"];

  $quais_pedagios = $row_todos["quais_pedagios"];

  $eixos_pedagios = $row_todos["eixos"];
...
<table width="750px" border="1" cellspacing="2" cellpadding="2" class="bordasimples">
  <tbody>
    <tr>
     <td height="40px" align="left" style="font-weight:bold;background-color: #F4F4F4;padding-left:8px;padding-right:10px;font-size:11px;"><font color="#000000">PEDÁGIOS: <?php echo $queme_pedagio; ?> </font></td>
...
<table width="750px" border="1" id="tabela" name="tabela" class="bordasimples" cellpadding="2" cellspacing="2">
...
<td > 
<div align="center">SENTIDO</div>
</td> 
</tr>

<?php
$pieces = explode( ',', trim( $quais_pedagios , ',' ) );
$count_pieces = count($pieces);

//print_r($pieces);
$soma_preco = 0;
for($q=0;$q<$count_pieces;$q++){

//echo $pieces[$q] . "<br>";

$query_tolls = mysql_query("SELECT * from pedagios where id='$pieces[$q]'");

$dados = mysql_fetch_array($query_tolls);

$nome_toll = $dados['nome'];
$end_toll = $dados['endereco'];
$preco_toll = $dados['preco']*$eixos_pedagios;
$preco_toll = number_format($preco_toll, 2, '.', ' ');
$km_toll = $dados['km'];
$sentido_toll = $dados['sentido'];

//$soma_com_eixo = $preco_toll * $eixos_pedagios;

$soma_preco += $preco_toll;
?>

<tr bgcolor="#FFF"  font color="#000000" >  
<td > 
<div><?php echo $nome_toll;  ?></div>
</td> 
<td > 
<div><?php echo $end_toll;  ?></div>
</td> 
<td > 
<div align="center"><?php echo $preco_toll;  ?></div>
</td>   
<td > 
<div align="center"><?php echo $km_toll;  ?></div>
</td>
<td > 
<div align="center"><?php echo $sentido_toll;  ?></div>
</td> 
</tr>
<?php
}
...
<td > 
<div align="center" style="color: #ff0000;"><?php echo "<b>" . number_format($soma_preco, 2, '.', ' ') . "</b>";  ?></div>
</td>   
<td > 
