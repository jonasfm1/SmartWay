...
if($string0==""){
    ...
    $completa2 = " AND setor in($string0) ";
}
...
if($string0==""){
    ...
    $completa1 = " AND setor_grupo in($string0) ";
}
...
$query ="SELECT * FROM clientes WHERE roteirizado!='sim' AND ativo=0 " . $completa0 . $completa1 . $completa2 .  $completa3 . " order by codigo DESC";
//echo $query;
...
<?php
$query1 = "select * from clientes where data_agendado!='0000-00-00' AND roteirizado!='sim' AND ativo=0" . $completa0 . $completa1 . $completa2 .  $completa3;															
$rs1 = mysql_query($query1);
...
//	$query_agrupar = "select * from clientes where codigo_cliente in(select B.codigo_cliente from clientes B group by B.codigo_cliente having count(*)>1)";
	$query_agrupar = "select *, COUNT(codigo_cliente) AS Qtd, SUM(peso) AS Qtd_peso, SUM(volume) AS Qtd_volume, SUM(valor) AS Qtd_valor from clientes where roteirizado!='sim' AND ativo=0 " . $completa0 . $completa1 . $completa2 .  $completa3 . "group by codigo_cliente having count(codigo_cliente)>1";
...
while($row = mysql_fetch_array($query_pontos)){
	...
	$lng_cad_pontos[$ie] = $row["longitude"];
        ...
        // ICONE ESPECIAL DA SEDE
                if ($nome_cad_pontos[$ie]=='SEDE COFIG'){
...
for ($i = 0; $i < $mrk_cnt; $i++){
		...
		} elseif ($premium[$i]=='000000') {
			$somaletra = 'R';
			$icone_google ="https://chart.googleapis.com/chart?chst=d_map_pin_letter&chld=". $somaletra ."|$premium[$i]|FFFFFF";
		} elseif ($especial[$i]=='1') {
			$somaletra = 'E';
			$icone_google ="https://chart.googleapis.com/chart?chst=d_map_pin_letter&chld=". $somaletra ."|$premium[$i]|000000";
		} elseif ($especial[$i]=='2') {
    	...
    	echo "marker[$i] = new google.maps.Marker({position: point$i, map: map,  label: {
		  ...
		  }, title: '$nome[$i]($codigo_pedido[$i]) • PESO: $peso[$i] • VOL.: $volume[$i]• VALOR: $valor[$i] • OBS.: $obs_pedido[$i] • DATA INCLUSÃO: $classificacao[$i] • CEP: $cep[$i]', icon: '$icone_google',});\n";
...
//////////////////////////////////////////////////////////////////////////////////// 
					...
					if(premium[i]!='' || premium[i]!="0"){
						...
						} else if(premium[i] == '000000') {
							somaletra = 'R';
							linkado = "https://chart.googleapis.com/chart?chst=d_map_pin_letter&chld=" + somaletra + "|" + premium[i] + "|FFFFFF";
						} else if(especial[i] == '1') {
							somaletra = 'D';
							linkado = "https://chart.googleapis.com/chart?chst=d_map_pin_letter&chld=" + somaletra + "|" + premium[i] + "|000000";
						} else if(especial[i] == '2') {
							somaletra = 'G';
							linkado = "https://chart.googleapis.com/chart?chst=d_map_pin_letter&chld=" + somaletra + "|" + premium[i] + "|000000";
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0" align="left" style="padding-top: 15px; padding-bottom: 8px; width:500px" >
		<tr>
			...
			if($veiculo1!=''){
				...
				$query = "select codigo, endereco, cidade, estado, cep, codigo_cliente, confianca_cad, latitude_cad, longitude_cad, endereco_cad, nome, veiculo, peso, volume, valor, codigo_pedido, obs_pedido, roteirizado, data_pedido, premium, data_agendado, obs_agendado from clientes where roteirizado!='sim' $completa0 $completa1 $completa2 $completa3 order by veiculo DESC, codigo_cliente ASC";																
				$rs = mysql_query($query);
				$totalderegistros = mysql_num_rows($rs);
...
<div align="center" nowrap='nowrap'>COD.PEDIDO</div>
</th>                                                       
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tbody>
				<tr>
					...
					<td width="85%" align="left" bgcolor="#589bd4" style="text-align: left; font-weight: bold; color: white;">&nbsp;DATA INCLUSÃO</td>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tbody> 
							<?php
								...
								$query_n = "select * from clientes where classificacao='$result[$i]' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2 . $completa3;	
							//	echo $query_n;															
								...
								<td align="right" style="text-align: center; width:12%"><input type="checkbox" id="<?php echo $result[$i];?>" name="classificacao[]" value="<?php echo $result[$i];?>"/></td>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0"  style="padding:0px">
			<tbody>
				<tr>
					...
					<td width="80%" align="left" bgcolor="#589bd4" style="text-align: left; font-weight: bold; color: white;">GRUPO SETOR</td>
			...
			<form method="POST" name="www" id="www">
				...
				<div id='layers1'>
					<table width="100%" border="0" cellspacing="0" cellpadding="0" style="padding-left: 5px;">
						<tbody> 
							<?php
								$query_vendedor = "select * from setor_grupo order by setor ASC";																
								$rs3_vendedor = mysql_query($query_vendedor);	
								$i=0;							
								while($row3_vendedor = mysql_fetch_array($rs3_vendedor)){

								$result_vendedor[] = $row3_vendedor["setor"];

								$query_n_vendedor = "select * from clientes where setor_grupo='$result_vendedor[$i]' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2 .  $completa3 ;																
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0" style="padding-left: 5px;">
						<tbody> 
							<?php
								...
								$query_n_setor = "select * from clientes where setor='$result_setor[$i]' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2 .  $completa3;																
							...
							//	echo $result[$i] . '(' . $cont . ')';
								...
								?></td>
								<td>
								<div style="background-color:#<?php echo $cor_setor[$i]; ?>; height:10px;width: 10px;margin-right:5px"></div>
								</td>
								<?php
								}
								?>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0"  style="padding:0px">
			<tbody>
				...
				<td width="20%" align="right" bgcolor="#589bd4" style="text-align: center; font-weight: bold;"><span class="material-icons" style="font-size: 16px;color:white">today </span></td>
					<td width="80%" align="left" bgcolor="#589bd4" style="text-align: left; font-weight: bold; color: white;">DIA</td>
				</tr>
			</tbody>
		</table>	
	</div>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0" style="padding-left: 5px;">
						<tbody> 
							<?php
								...
								$query_n_dia = "select * from clientes where $dia_abreviado='1' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2.  $completa3;																
...
<div class ="container" id="makeMeDraggable" name="makeMeDraggable" onmouseout="return false"> 
	<div class="header">
		<table width="100%" border="0" cellspacing="0" cellpadding="0"  style="padding:0px">
			<tbody>
				<tr>
				<td align="right" bgcolor="#589bd4" style="text-align: left; font-weight: bold;" height="32">&nbsp;</td>
				<td width="20%" align="right" bgcolor="#589bd4" style="text-align: center; font-weight: bold;"><span class="material-icons" style="font-size: 16px;color:white">map </span></td>
					<td width="80%" align="left" bgcolor="#589bd4" style="text-align: left; font-weight: bold; color: white;">LAYER</td>
			</tr>
			</tbody>
		</table>	
	</div>  
    <div class="content" id='legendas_layer'>
		...
		<tr>
			...
			//echo $pega_nome; 
			echo mb_strimwidth($pega_nome, 0, 15, "...");
			?></td>
			</tr>
...
<?php
  ...
  <tr bgcolor="#ECECEC" style="text-align: center;" height="34" >
    ...
    <td title="VALOR RESTANTE"><font color="#000000" style="font-weight:bold;">R/VALOR</font></td>
  </tr>
