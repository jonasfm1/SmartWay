...
$deleterecords2 = "DELETE FROM setor_grupo"; //Esvaziar a tabela classificacao
mysql_query($deleterecords2);

//$deleterecords3 = "DELETE FROM setor"; //Esvaziar a tabela classificacao
...
/// TITULOS ////////////////
  ...
  $motivo_reentrega = array_search("Codigo Interno Nota", $array_titulo); // $key = 36;

  $gruposetor = array_search("Ds Gp Setor Entrega", $array_titulo); // $key = 9;

  $agenda_icon = array_search("DATAHORASTATUS", $array_titulo); // $key = 4;

  $setor = array_search("Ds Setor Entrega", $array_titulo); // $key = 9;


  $seg = array_search("Recebe Segunda", $array_titulo); //
  $ter = array_search("Recebe Terca", $array_titulo); //
  $qua = array_search("Recebe Quarta", $array_titulo); //
  $qui = array_search("Recebe Quinta", $array_titulo); //
  $sex = array_search("Recebe Sexta", $array_titulo); //
  $sab = array_search("Recebe Sabado", $array_titulo); //
  $dom = array_search("Recebe Domingo", $array_titulo); //

  $tipo_cli = array_search("Perfil De Risco Do Cliente", $array_titulo); //
...
$data_setor_grupo = $rowData[0][$gruposetor];
if($data_setor_grupo==""){
  $data_setor_grupo = "INEXISTENTE";
}

$data_setor = $rowData[0][$setor];

if($data_setor==""){
  $data_setor = "INEXISTENTE";
}

// DIA SEMANA
$data_seg = $rowData[0][$seg];
$data_ter = $rowData[0][$ter];
$data_qua = $rowData[0][$qua];
$data_qui = $rowData[0][$qui];
$data_sex = $rowData[0][$sex];
$data_sab = $rowData[0][$sab];
$data_dom = $rowData[0][$dom];
//

// TIPO CLIENTE

$data_tipo_cliente = $rowData[0][$tipo_cli];
//
...
//$data_agenda = $rowData[0][$agenda_icon];	//AGENDA ICON
...
// NAO TEM NO CLIENTES
      ...
      // SE É UMA REENTREGA
      if($data_reentrega!='' OR $data_reentrega!=null){
        $premium = '000000';
        ...
        $import="INSERT into clientes(codigo_cliente, nome, endereco, cidade, estado, cep, peso, volume, valor, codigo_pedido, obs_pedido, premium, classificacao, bairro, data_pedido, setor_grupo, data_agendado, setor, nota, seg, ter, qua, qui, sex, sab, dom, especial)values('$data_0','$data_1','$data_2','$data_3','$data_4','$data_5','$data_6', '$volume', '$data_7', '$data_8', '$data_9', '$premium', '$data_13', '$data_12', '$data_13', '$data_setor_grupo', '$data_agenda', '$data_setor', '$data_reentrega', '$data_seg', '$data_ter', '$data_qua', '$data_qui', '$data_sex', '$data_sab', '$data_dom', '$data_tipo_cliente')";
...
// SETOR GRUPO/////////////
$query_setor = "select setor_grupo from clientes group by setor_grupo";														
$rs_setor = mysql_query($query_setor);

while ($row_setor = mysql_fetch_array($rs_setor)) {
   $ve =  $row_setor['setor_grupo'];

//echo $row_classifica['classificacao'];

$query_ve = "INSERT into setor_grupo(setor) values ('$ve')";
$query_ve= mysql_query($query_ve);

}

// TABELA CORES
$query_cores_pega  = "SELECT cor from color";
$rs_cores_pega = mysql_query($query_cores_pega);
...
while ($row_cores = mysql_fetch_array($rs_cores_pega)) {
  $lista_cores =  $row_cores['cor'];
  array_push($array_cores, $lista_cores);
}
...
// SETOR /////////////
$query_setor1 = "select setor from clientes group by setor";														
$rs_setor1 = mysql_query($query_setor1);

while ($row_setor1 = mysql_fetch_array($rs_setor1)) {


   $ve1 =  $row_setor1['setor'];

   $query_verifica = mysql_query("select setor, color from setor where setor='$ve1'");
   $total_verifica = mysql_num_rows($query_verifica);

   // Tirando o while
   $dados = mysql_fetch_array($query_verifica);
...
if($total_verifica!=0){
  ...
  $query_updata_setor = mysql_query("UPDATE clientes SET premium='$cor' where setor='$ve1' AND data_agendado='0000-00-00' AND nota=''");

} else {

  $rand_keys = array_rand($array_cores, 1);

  
  $query_ve1 = "INSERT into setor(setor, color) values ('$ve1','$array_cores[$rand_keys]')";
  $query_ve1= mysql_query($query_ve1);

  // COR ICONE

  $query_updata_setor = mysql_query("UPDATE clientes SET premium='$array_cores[$rand_keys]' where setor='$ve1' AND data_agendado='0000-00-00' AND nota=''");
  $query_del_cor = mysql_query("DELETE from color where cor='$array_cores[$rand_keys]'");

  unset($array_cores[$rand_keys]);
...
<table border="0">
<tr style="height: 15px; vertical-align: button">
<td>
...
<table border="0">
<tr style="height: 10px; vertical-align: center">
<td>
<strong><font size="5">
...
<tr style="height: 20px;vertical-align: button">
<td><strong><font size="4">CHECK-LIST!</font></strong></td>
</tr>
<tr style="height: 25px;vertical-align: top">
<td><hr style="border: none; width:110px;" ></td>
...
<td><p style="font-size:12px"><strong><?php echo $abreviado; ?> LIDO!</p></td>
...
if ($_conta==0){
?>
<strong style="color:red">&nbsp;<?php echo $_conta; ?></strong>
<?php 
} else {
?>
<strong style="color:green">&nbsp;<?php echo $_conta; ?></strong>
<?php 
}
...
<td><p style="font-size:12px">PEDIDOS REENTREGAS:</p></td>
<td><strong style="color:green">&nbsp;<?php echo $conta_reentrega; ?></strong></td>
</tr>
<tr style="height: 20px;">

<td><p style="font-size:12px">PEDIDOS AGENDADOS:</p></td>
<td><strong style="color:green">&nbsp;<?php echo $conta_agendado; ?></strong></td>
</tr>
<tr style="height: 20px;">
