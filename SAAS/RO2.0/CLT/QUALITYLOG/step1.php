...
if (isset($_POST['submit'])) {
	...
	*/
	...
	<div id="loading" style="position:absolute; width:100%; height:100%; background-color:#FFF; z-index:99999; top:0;">
	<div style="height:100px;background-color:#FFF; width:400px; position: relative; top: 50%; margin-top:-50px; left:50%; margin-left:-200px">
	...
	while (($data = fgetcsv($handle, 1000, ';')) != FALSE) {
		...
		$cod_cli = str_replace("'", "`", $data[2]);	//COD CLIENTE

		$nome_cli = str_replace("'", "`", $data[1]); 	//NOME CLIENTE
		$nome_cli = tirarAcentos($nome_cli);
		$nome_cli = str_replace("´", " ", $nome_cli);
		$nome_cli = str_replace("`", " ", $nome_cli);
		$nome_cli = str_replace("ç", "c", $nome_cli);
		$nome_cli = str_replace("Ç", "C", $nome_cli);
		$nome_cli = preg_replace( "/\r|\n/", "", $nome_cli );
		$nome_cli = trim($nome_cli);



		$peso_cli = str_replace(",", ".", $data[6]);	//PESO
		$volume_cli = str_replace(",", ".", $data[5]);	//VOLUME
		$valor_cli = str_replace(",", ".", $data[7]);	//VALOR

		$cod_pedido_cli = str_replace(",", ".", $data[0]);	//CODIGO DO PEDIDO

		$obs_cli = str_replace(",", ".", $data[4]);	//OBS DO PEDIDO
		$obs_cli = tirarAcentos($obs_cli);
		$obs_cli = str_replace("´", " ", $obs_cli);
		$obs_cli = str_replace("`", " ", $obs_cli);
		$obs_cli = str_replace("ç", "c", $obs_cli);
		$obs_cli = str_replace("Ç", "C", $obs_cli);
		$obs_cli = preg_replace( "/\r|\n/", "", $obs_cli);
		$obs_cli = trim($obs_cli);

		$cod_produto = str_replace(",", ".", $data[3]);	//PRODUTO
	...
	//	$data_0 = utf8_encode($data_0);
		...
		$premium = '';
			...
			if ($peso_cli>=200 AND $peso_cli<300){
			$premium = 'V';
				
			}
			if ($peso_cli<200 AND $peso_cli>=30){
			$premium= 'S';
	...
	///////////////////////////////////////////////////////////////////////
		...
		$import="INSERT into clientes(codigo_cliente, nome, peso, volume, valor, codigo_pedido, cod_produto, produto, premium)values('$cod_cli','$nome_cli','$peso_cli','$volume_cli','$valor_cli','$cod_pedido_cli', '$cod_produto', '$obs_cli','$premium')";
	...
	$query = "select * from db_geral where codigo_cliente='$cod_cli'";														
	...
	$dados = mysql_fetch_array($rs);
				...
				$cep_cli= $dados["cep"];
				$cidade_cli= $dados["cidade"];
				$estado_cli= $dados["estado"];

	
				//echo $uf;
	
		if($id_cliente == $cod_cli){	
				$conta_regs_existentes++;
				//echo $cidade_cli;
				
	   $query2 = "UPDATE clientes SET latitude_cad='$lat_cad', longitude_cad='$lgn_cad', confianca_cad='$confianca_cad', codigo_db_geral='$codigo', endereco='$end_cad', endereco_cad='$end_cad', cep='$cep_cli', cidade='$cidade_cli', estado='$estado_cli' WHERE codigo_cliente = '$id_cliente'";
	...
	//$query_rtrim = "UPDATE clientes SET nome = LTRIM($nome), endereco = TRIM($endereco)"; 
...
<td><p style="font-size:12px">&nbsp;&nbsp;INSERIR NOVA LISTA DE PEDIDOS!</p></td>
</tr>
