...
date_default_timezone_set('Etc/GMT+3');
    ...
    while ($row20 = mysql_fetch_array($rs20)) {
        ...
        //////////////////////////////// SLT ///////////////////////////////////////////////////////////
            ...
            $query2 = "INSERT INTO rotas(login, rota, sequencia, x, y, idcliente, nome, endereco, cidade, classificacao, tempo, obs, raio, status, statusrota, lista, codigo_pedido, peso, volume, valor, site) VALUES('$login', '$rota', '$ordem', '$x', '$y', '$codigo_cliente', '$nome_cliente', '$endereco', '$cidade', '$codigo_pedido', '$tempo_servico', '$obs_pedido', '$raio', '$status', 1, '$lista', '$codigo_pedido', $peso, $volume, $valor, 1)";
            $rs2 = mysql_query($query2);
...
foreach ($lista_setor as $v) {
	...
	$veiculos_setores = "select * from veiculos where roteirizado!='sim' order by custo ASC";
	...
	$clientes_setor = "select * from clientes where setor='$v' AND roteirizado!='sim' order by cep, bairro, codigo_cliente, cidade ASC";
	...
	while($row_clientes_setor = mysql_fetch_array($clientes_setor)){
		 ...
		 $lat = $row_clientes_setor["latitude_cad"];
		 $lgn = $row_clientes_setor["longitude_cad"];
		


		 $lista_clientes = array($codigo_cliente2,$nome2,$peso2,$volume2,$valor2,$lat,$lgn);
...
/*
	...
	while($x < $maximo){
		...
		if ($x>0){
			...
			$haversign1=new HaverSign();
		$dist1 = $haversign1->getDistance($allplayerdata2[0][5],$allplayerdata2[0][6],$allplayerdata2[$x][5],$allplayerdata2[$x][6]);

		array_push($allplayerdata2[$x][7] = $dist1);

			
		} else {

			array_push($allplayerdata2[$x][7] = 0);
		}

		
		

		//echo $allplayerdata2[$i2-1][6];
		echo $dist1;
		echo "<br>";
...
//	echo "<br>";
	...
	$lat_pega = $allplayerdata[0][5];
	$lgn_pega = $allplayerdata[0][6];
	...
	while($i2 < $max2){
			...
			$dist = $haversign->getDistance($allplayerdata2[$i2-1][5],$allplayerdata2[$i2-1][6],$allplayerdata2[$i2][5],$allplayerdata2[$i2][6]);
			...
			//echo "<br>";
			//print_r($allplayerdata2);
			if($dist>100 and $i2>0){
			echo "<br><br>";
			echo ("PASSOU A DISTANCIA!");
			echo "<br><br>";
		...
		if($peso_veiculo<=0 or $volume_veiculo<=0 or $valor_veiculo<=0 or ($dist>100 and $i2>0)){
...
//// LOADING 0 A 100% ////////////////////////////////////////////////////
    ...
    echo "<div style='position:absolute;margin:0px 0px 0px 0px;width:$pega_loading;height:40px;background:#2867b2;color: #FFFFFF;text-align:center;line-height:40px;'>$pega_loading</div>";
    flush();
    ob_flush();
