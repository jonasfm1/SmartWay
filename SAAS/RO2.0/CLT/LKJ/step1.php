...
?> 
	...
	while (($data = fgetcsv($handle, 1000, ';')) != FALSE) {
		...
		if ($_conta >= 0) {
			...
			$data_1 = str_replace("'", "`", $data[2]); 	//NOME CLIENTE
			$data_1 = tirarAcentos($data_1);
			$data_1 = str_replace('´', '', $data_1); 	//NOME CLIENTE
			$data_1 = str_replace('"', '', $data_1); 	//NOME CLIENTE
			
			$data_2 = str_replace("'", ",", $data[3]);	//ENDERECO
			$data_2 = tirarAcentos($data_2);
			$data_2 = str_replace('´', "", $data_2);	//ENDERECO
			$data_2 = str_replace('"', "", $data_2);	//ENDERECO
			$numero_end = str_replace("'", ",", $data[3]);	//numero
		
			$end_completo = $data_2 . ", " . $numero_end;
		
			$bairro = str_replace("'", ",", $data[5]);	//bairro
			
			$data_3 = str_replace("´", " ", $data[6]); //CIDADE
			$data_3 = tirarAcentos($data_3);
			$data_3 = str_replace("'", " ", $data_3);	//CIDADE
			$data_3 = str_replace('"', " ", $data_3);	//CIDADE
	
			
			$data_4 = str_replace("'", "`", $data[7]);	//UF
			$data_4 = str_replace('´', " ", $data_4);	//UF
			$data_4 = str_replace('"', " ", $data_4);	//UF
			
			$data_5 = str_replace("'", "`", $data[8]);	//CEP
		
			$data_6 = str_replace(",", ".", $data[9]);	//PESO
			$data_7 = str_replace(",", ".", $data[10]);	//VOLUME
			$data_8 = str_replace(",", ".", $data[11]);	//VALOR
			
			$data_9 = str_replace(",", ".", $data[12]);	//CODIGO DO PEDIDO
			$data_10 = str_replace(",", ".", $data[13]);	//OBS DO PEDIDO
		...
		//LKJ/// VERIFICA SE TEM CODIGO DO PEDIDO  IGUAL NA BASE CLIENTES
	

		$query_where_verifica =  mysql_query("SELECT codigo_pedido from clientes where codigo_pedido='$data_9'");
...
//////////////////////////////////////////////////////////////////////////////////////////
	...
	if($num_rows>0){
			...
			$import="INSERT into clientes(codigo_cliente, nome, endereco, cidade, estado, cep, peso, volume, valor, codigo_pedido, obs_pedido, setor, premium, bairro)values('$data_0','$data_1','$data_2','$data_3','$data_4','$data_5','$data_6','$data_7','$data_8','$data_9','$data_10','$data_11','$premium','$bairro')";
...
<td><strong style="color:red">&nbsp;<?php echo $conta_menos_primeiro; ?></strong></td>
</tr>
...
<td><p style="font-size:12px">NÚMEROS DE REGISTROS INSERIDOS:</p></td>
<td><strong style="color:green">&nbsp;<?php echo $conta_menos_primeiro; ?></strong></td>
</tr>
