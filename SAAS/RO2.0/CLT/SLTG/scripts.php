...
//////////////ADICONA REDESPACHO BD GERAL////////////////////////////////

case 'redespacho':
	$lat = $_POST["new_lat_2"];
	$lgn = $_POST["new_lgn_2"];
	$endereco = $_POST["new_end_2"];
	$cidade = $_POST["cidade"];
	$uf = $_POST["uf"];
	$cep = $_POST["cep"];
	$bairro =$_POST["bairro"];
	...
	$query1 = "SELECT codigo_cliente from db_geral where codigo_cliente='$cnpj'";
...
?>
	...
	<td><img src="imgs/erro.PNG"/>	<br>
	<strong>>REDESPACHO NÃO INSERIDO!</strong>	
		</td>
	</tr>

</table>
<SCRIPT LANGUAGE="JavaScript" TYPE="text/javascript"> 
	alert("Cliente já encontrado no banco de dados. Por favor, insira um CNPJ diferente.");

	setTimeout(parent.location.reload(), 1000000);
...
//echo "nao tem";
$query = "INSERT INTO db_geral(codigo_cliente, latitude_cad, longitude_cad, nome, endereco, endereco_cad, cidade, estado, cep, bairro, confianca_cad) VALUES ('$cnpj', $lat, $lgn, '$nome', '$endereco', '$endereco','$cidade', '$uf', '$cep', '$bairro', '101')";
...
<table width="100%" height="100%" style="background-color:#FFF; vertical-align: middle; text-align: center">
	...
	<td><img src="imgs/ok.PNG"/>
		<br>
	<strong>REDESPACHO INSERIDO COM SUCESSO!</strong>	
		</td>
	</tr>

</table>
<SCRIPT LANGUAGE="JavaScript" TYPE="text/javascript">
alert("REDESPACHO INSERIDO COM SUCESSO!");
	setTimeout(parent.location.reload(), 1000000);
...
/////////////// DISABLE CLIENTES ///////////////////////
	...
	case 'importar_monitoramento':

		$bd = "mo_slt";
	
		$bd_galpao = "mo_slt_galpao";
	...
	date_default_timezone_set('Etc/GMT+3');
		...
		//////////////////////////////// SLT ///////////////////////////////////////////////////////////
	if ($pega_slt == 'slt_gsp1' or $pega_slt == 'slt_gsp2' or $pega_slt == 'slt_i1' or $pega_slt == 'slt_galpao') {
			$new = "ro_" . $pega_slt;
			...
			if ($pega_slt == 'slt_galpao') {
				$quem = " - SLT GALPAO - ";
	
			}
	
	
			$lista = $dataPedido . $quem . " (" . $today1 . ")";	
	...
	//echo $lista;
		...
		while ($row20 = mysql_fetch_array($rs20)) {
			...
			$volume = $row20['volume'];
			$valor = $row20['valor'];	
			$motorista = $row20['motorista'];	
			$raio = 300;
			$status = 0;
			$remetente = $row20['remetente'];
	...
	//echo $login . "<br>";
			...
			if($pega_slt == 'slt_galpao'){
				$select2 = mysql_select_db($bd_galpao) or die("Sem acesso ao DB, Entre em contato com o Administrador!");
				...
				$query2 = "INSERT INTO rotas(redespacho, remetente, login, rota, sequencia, x, y, idcliente, nome, endereco, cidade, classificacao, tempo, obs, raio, status, statusrota, lista, codigo_pedido, peso, volume, valor, motorista, site, data_teste) VALUES('$redespacho', '$remetente', '$login', '$rota', '$ordem', '$x', '$y', '$codigo_cliente', '$nome_cliente', '$endereco', '$cidade', '$codigo_pedido', '$tempo_servico', '$obs_pedido', '$raio', '$status', 1, '$lista', '$codigo_pedido', $peso, $volume, $valor, '$motorista', 1, '$today_usa')";
				$rs2 = mysql_query($query2);
    ...
    <div class="modal-content">
      <div class="modal-header">
		  <table>
			  <tr>
				  <td>
				  <span class="material-icons" style="font-size: 48px;color:#2867b2">app_settings_alt</span>
				  </td>
				  <td>
				 <strong><p class="modal-title" id="exampleModalLongTitle" style="text-align: left;">&nbsp;&nbsp;IMPORTAR MONITORAMENTO</p></strong> 
				  </td>
			  </tr>
		  </table>
	  ...
	  &nbsp;&nbsp;Lista '<?php echo $lista ?>' importada com sucesso!
      </div>
...
case 'bloqueia_veiculos_todos_g':
	...
	$query_ativa = "UPDATE veiculos SET roteirizado='' WHERE ocupado=1";
...
while($row_agrupar = mysql_fetch_array($query_agrupar)){
	...
	while($row_agrupar1 = mysql_fetch_array($query_agrupar1)){
		...
		$query_adiciona_lgn = "UPDATE clientes SET longitude_cad='$lgn_muda' WHERE codigo='$pega_codigo_geral'";															
...
while($row3 = mysql_fetch_array($rs3)){
  ...
  $cnpj_redespacho = $row3["cnpj_redespacho"];
...
*/
		...
		if(@mysql_num_rows($search) > 0){
			...
			if($cnpj_redespacho==''){
				$sql = mysql_query("UPDATE db_geral SET nome='$nome', endereco='$endereco', cidade='$cidade', estado='$estado', endereco_cad='$endereco_cad', latitude_cad='$latitude_cad', longitude_cad='$longitude_cad', confianca_cad='101', bairro='$bairro' WHERE codigo_cliente = '$codigo_cliente'");
			}
...
//	$cod_certo= $row["codigo_cliente"];
//	echo $cod_certo;
//	echo "xxxx";

 $query2 = "UPDATE clientes SET nome='$nome_cliente', endereco='$endereco_cliente', bairro='$bairro_cliente', cidade='$cidade_cliente', estado='$uf_cliente', cep='$cep_cliente' WHERE codigo_cliente='$cod_cliente'";
  $rs2= mysql_query($query2);
...
case 'libera_veiculo_g':
	...
	//echo $id_veiculo ;
		...
		$query_ativa = "UPDATE veiculos SET roteirizado=null WHERE id = '$id_veiculo'";
		$rs_ativa = mysql_query($query_ativa);
...
$query_veiculos = "UPDATE veiculos SET distancia_total='$i_distancia_total[$w]', tempo_total='$i_tempo_total[$w]', local_inicio='$inicio', local_final='$final' WHERE id = '$i_veiculo_total[$w]'";
$rs_veiculos = mysql_query($query_veiculos);
...
if($transp==''){
	...
	$query3 = "UPDATE rotas SET veiculo='$i_id_veiculo[$y]', transportadora='$transp', nome_rota='$somadata_rota', ordem='$i_ordem[$y]', distancia='$i_distancia[$y]', tempo='$i_tempo[$y]', tempo_servico='$data_hoje' WHERE id = '$i_id_cliente[$y]'";
...
case 'limpa_filtro_step3':
	...
	//print_r($_SESSION["classificacao"]) ;
