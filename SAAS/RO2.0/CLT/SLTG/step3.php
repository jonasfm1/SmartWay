...
$query ="SELECT * FROM clientes WHERE roteirizado!='sim'";
...
while($row = mysql_fetch_array($rs)){
	...
	$redespacho[$mrk_cnt] = $row["redespacho"];
	$ocupado[$mrk_cnt] = $row["veiculo"];
	$rmt[$mrk_cnt] = $row["remetente"];
...
$query_agrupar1 = "select *, COUNT(cnpj_redespacho) AS Qtd1, SUM(peso) AS Qtd_peso1, SUM(volume) AS Qtd_volume1, SUM(valor) AS Qtd_valor1 from clientes where roteirizado!='sim' group by cnpj_redespacho";
$query_agrupar1 = mysql_query($query_agrupar1);

$iex1=5000000000;

while($row_x1 = mysql_fetch_array($query_agrupar1)){
	$i_conta1++;
	
	$icon_novo_nome1[$iex1] = $row_x1["redespacho"];
	$icon_novo1[$iex1] = $row_x1["codigo_cliente"];
	$icon_novo_lat1[$iex1] = $row_x1["latitude_cad"];
	$latitudeponto =  $icon_novo_lat1[$iex1];
	$latitudeponto = $latitudeponto + 0.000012;
	$icon_novo_lgn1[$iex1] = $row_x1["longitude_cad"] ;
	$longitudeponto = $icon_novo_lgn1[$iex1];
	$longitudeponto = $longitudeponto + 0.000002;


	/*
	$novodigito =  intval($novodigito);
	
	if($novodigito == 0){
		$novodigito = $novodigito + 1 ;
	}elseif($novodigito == 1){
		$novodigito = $novodigito + 1 ;
	}else{
		$novodigito = $novodigito - 1;
	}
	$novo = substr($icon_novo_lat1[$iex1], 0, 8);
	$novo = $novo . $novodigito;
	*/
	
	//echo $icon_novo_lat1[$iex1] . "|" . $icon_novo_lgn1;
	//echo $novo . "aaaaa";
	$quanto1 = $row_x1["Qtd1"];
	$quanto_peso1 = $row_x1["Qtd_peso1"];
	$quanto_volume1 = $row_x1["Qtd_volume1"];
	$quanto_valor1 = $row_x1["Qtd_valor1"];
	$cnpj_redespacho = $row_x1["cnpj_redespacho"];
	if($cnpj_redespacho!=""){
			//echo $icon_novo[$ie];
	echo "pointXy$iex1 = new google.maps.LatLng($latitudeponto,$longitudeponto);\n";	
	echo "mrktxXy$iex1 = \"<div id='box_map'><h1><p id='legenda2'>$icon_novo_nome1[$iex1]</div>\";";
	echo "infowindowXy$iex1 = new google.maps.InfoWindow({content: mrktxXy$iex1});\n";
	echo "marker$iex1 = new google.maps.Marker({label:''+$quanto1, position: pointXy$iex1, map: map, title: ''+'$icon_novo_nome1[$iex1]'+' • PESO: '+$quanto_peso1+' • VOLUME: '+$quanto_volume1+' • VALOR: '+$quanto_valor1 , zIndex: 5200,  icon: 'imgs/alert_icon_redespacho.png',});\n";
	//echo "marker$iex = nnew google.maps.MarkerImage('{{STATIC_URL}}img/icon-pin'+($iex)+'.png');\n";

	echo "google.maps.event.addListener(marker$iex1,
	 'click',  function() {
			infowindowXy$iex1.open(map,marker$iex1);
	  });\n";
	}
...
//	echo "aMapLabel = new MapLabel({text: '+$quanto, position: new google.maps.LatLng(pointXy$iex),strokeColor: '#000000', map: map, fontSize: 15, minZoom: 16,strokeWeight: 10, fillWeight: 5, zIndex: 5200,align: 'center'});\n";
	...
	$query_agrupar = "select *, COUNT(codigo_cliente) AS Qtd, SUM(peso) AS Qtd_peso, SUM(volume) AS Qtd_volume, SUM(valor) AS Qtd_valor from clientes where roteirizado!='sim' group by codigo_cliente having count(codigo_cliente)>1";
	...
	while($row_x = mysql_fetch_array($query_agrupar)){
    	...
    	echo "marker$iex = new google.maps.Marker({label:''+$quanto, position: pointXy$iex, map: map, title: ''+'$icon_novo_nome[$iex]'+' • PESO: '+$quanto_peso+' • VOLUME: '+$quanto_volume+' • VALOR: '+$quanto_valor , zIndex: 200,  icon: 'imgs/alert_icon.png',});\n";
		//echo "marker$iex = nnew google.maps.MarkerImage('{{STATIC_URL}}img/icon-pin'+($iex)+'.png');\n";
...
for ($i = 0; $i < $mrk_cnt; $i++){
		...
		//$mrk_cnt2 = 0;
		//////////////////////////////////
		if($lng_cad[$i] == $lng_cad[$i-1]){
			$lng_cad[$i] = $lng_cad[$i] + 0.00001;
		}
	  	echo "point$i = new google.maps.LatLng($lat_cad[$i],$lng_cad[$i]);\n";
