...
<?php
	...
	$deleteremontar = "TRUNCATE TABLE remontar"; //Esvaziar a tabela
	mysql_query($deleteremontar);
	...
	if(!empty($_GET['check_list1'])){
     foreach($_GET['check_list1'] as $report_id1){
		...
		while($row_fora = mysql_fetch_array($rs_fora)){
			...
			$query3_fora = "INSERT INTO rotas(codigo_cliente, nome_cliente, lat, lgn, veiculo, endereco, cep, cidade, ordem, codigo_pedido, obs_pedido, peso, volume, valor, nome_rota, setor)

			VALUES('$codigo_cliente_fora[$conta_fora1_new]', '$nome_cliente_fora[$conta_fora1_new]', '$lat_cliente_fora[$conta_fora1_new]', '$lgn_cliente_fora[$conta_fora1_new]', '$veiculo_cliente_fora[$conta_fora1_new]',
			'$endereco_cliente_fora[$conta_fora1_new]', '$cep_cliente_fora[$conta_fora1_new]', '$cidade_cliente_fora[$conta_fora1_new]', '0', '$codigo_pedido_fora[$conta_fora1_new]', '$obs_pedido_fora[$conta_fora1_new]',
			'$peso_cliente_fora[$conta_fora1_new]', '$volume_cliente_fora[$conta_fora1_new]', '$valor_cliente_fora[$conta_fora1_new]', '$agrupamento_concatena', '$setor_cliente_fora')";
...
if(!empty($_GET['check_list'])){
     foreach($_GET['check_list'] as $report_id){
		...
		while($row = mysql_fetch_array($rs1)){
			...
			$endereco_redespacho[$conta1_new] = $row["endereco_redespacho"];
			$cidade_redespacho[$conta1_new] = $row["cidade_redespacho"];
			$uf_redespacho[$conta1_new] = $row["uf_redespacho"];
			$redespacho[$conta1_new] = $row["redespacho"];
			$remetente[$conta1_new] = $row["remetente"];
			
			$especial[$conta1_new] = $row["especial"];
			//echo $veiculo_cliente[$conta1];

			//	INOFRMAÇÕES NECESSARIAS PARA A TABELA REMONTAR
			$serie[$conta1_new] = $row["serie"];
			$dt_emissao[$conta1_new] = $row["dt_emissao"];
			$cnpj_remetente[$conta1_new] = $row["cnpj_remetente"];
			$cnpj_destinata[$conta1_new] = $row["cnpj_destinatario"];
			$estado_cli[$conta1_new] = $row["estado"];
			$cnpj_redespacho[$conta1_new] = $row["cnpj_redespacho"];
			$cep_redespacho[$conta1_new] = $row["cep_redespacho"];

			$uf = $row["estado"];


			$query3 = "INSERT INTO rotas(codigo_cliente, nome_cliente, lat, lgn, veiculo, endereco, bairro, cep, cidade, codigo_pedido, obs_pedido, tempo_servico, peso, volume, valor, setor, endereco_redespacho, cidade_redespacho, especial, uf_redespacho, redespacho, remetente, nome_veiculo, tipo_veiculo, uf)
			VALUES('$codigo_cliente[$conta1_new]', '$nome_cliente[$conta1_new]', '$lat_cliente[$conta1_new]', '$lgn_cliente[$conta1_new]', '$veiculo_cliente[$conta1_new]', '$endereco_cliente[$conta1_new]', '$bairro_cliente[$conta1_new]', '$cep_cliente[$conta1_new]', '$cidade_cliente_fora[$conta1_new]',
			'$codigo_pedido[$conta1_new]', '$obs_pedido[$conta1_new]', '$temposervico_cliente[$conta1_new]', '$peso_cliente[$conta1_new]', '$volume_cliente[$conta1_new]', '$valor_cliente[$conta1_new]', '$setor_cliente[$conta1_new]', '$endereco_redespacho[$conta1_new]', '$cidade_redespacho[$conta1_new]',
			'$especial[$conta1_new]', '$uf_redespacho[$conta1_new]','$redespacho[$conta1_new]','$remetente[$conta1_new]', '$nome_veiculo', '$tipo', '$uf')";
			$rs3 = mysql_query($query3);


			$queryremonta = "INSERT INTO remontar (n_nota, serie, peso_bruto, quant_carga, tot_nota, dt_faturamento, cnpj_rmt, nome_rmt, cnpj_cli, nome_cli,cidade_cli, endereco_cli, bairro_cli, estado_cli, cep_cli, 
			cnpj_transp, nome_transp, cid_transp, end_transp, uf_transp, cep_transp, nfe, veiculo) VALUES('$codigo_pedido[$conta1_new]', '$serie[$conta1_new]', '$peso_cliente[$conta1_new]', '$volume_cliente[$conta1_new]', '$valor_cliente[$conta1_new]', '$dt_emissao[$conta1_new]',
			'$cnpj_remetente[$conta1_new]', '$remetente[$conta1_new]', '$cnpj_destinata[$conta1_new]', '$nome_cliente[$conta1_new]', '$cidade_cliente_fora[$conta1_new]', '$endereco_cliente[$conta1_new]', '$bairro_cliente[$conta1_new]', '$estado_cli[$conta1_new]', '$cep_cliente[$conta1_new]',
			'$cnpj_redespacho[$conta1_new]', '$redespacho[$conta1_new]', '$cidade_redespacho[$conta1_new]', '$endereco_redespacho[$conta1_new]','$uf_redespacho[$conta1_new]', '$cep_redespacho[$conta1_new]', '$obs_pedido[$conta1_new]', '$veiculo_cliente[$conta1_new]')";
			$execremonta = mysql_query($queryremonta);
