...
if ($pega_listagem == "orange") {
  ...
  if($pega_escolha==""){   
    $query = "select codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, premium, empresa, codigo_pedido, marca, loja from clientes where confianca_cad>50 AND confianca_cad<=70 AND roteirizado!='sim' and ativo=0 group by codigo_cliente";
...
if ($pega_listagem == "green") {
  ...
  if($pega_escolha==""){   
    $query = "select codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, premium, empresa, codigo_pedido, marca, loja from clientes where confianca_cad>70 AND confianca_cad<=100 AND roteirizado!='sim' and ativo=0 group by codigo_cliente";
...
if ($pega_listagem == "blue") {
  ...
  if($pega_escolha==""){   
    $query = "select codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, premium, empresa, codigo_pedido, marca, loja from clientes where confianca_cad=101 AND roteirizado!='sim' and ativo=0 group by codigo_cliente";
    } else {
...
if ($pega_listagem == "roxo") {
  ...
  if($pega_escolha==""){   
  $query = "select codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, premium, empresa, codigo_pedido, marca, loja from clientes where roteirizado!='sim' and ativo=1 group by codigo_cliente";
  } else {
...
<div align="center" >COD.PEDIDO</div>
</th>
...
?>
    ...
    <td align="center" nowrap="nowrap" width="auto" style="width: 20px;text-align: center;"><input type="checkbox" class="marcar" name="check_list[]" id="check_list" value="<?php echo $row["codigo_cliente"]; ?>"></td>
      ...
      ?>
        ...
        ?></td>
        <td align="center" id="id" nowrap="nowrap"><strong><?php 
        ...
        <td align="left" title="<?php echo $row['empresa'];?>" nowrap="nowrap"  ><?php 
        echo $row["empresa"];
       ...
       // echo $row["cep"] 
        ...
        ?></td>
        <td align="center" nowrap="nowrap" title='<?php echo $row['marca']; ?>'><?php 
        //echo $row["peso"] 
        echo mb_strimwidth($row["marca"], 0, 9, "...");
        ?></td> 
        <td align="center" nowrap="nowrap" title='<?php echo $row['loja']; ?>'><?php 
        ///echo $row["volume"] 
        echo mb_strimwidth($row["loja"], 0, 9, "...");
        ?></td>
        ...
        <td align="center" title='<?php echo $row['codigo_pedido']; ?>'><?php 
        //echo $row["premium"] 
        echo mb_strimwidth($row["codigo_pedido"], 0, 9, "...");
        ?></td>
