...
if ($pega_listagem == "red" or $pega_listagem == "") {
  ...
  if($pega_escolha==""){   
    $query = "select codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, premium, ref from clientes where confianca_cad<=50 AND roteirizado!='sim' AND ativo=0 group by codigo_cliente";
...
if ($pega_listagem == "orange") {
  ...
  if($pega_escolha==""){   
    $query = "select codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, premium, ref from clientes where confianca_cad>50 AND confianca_cad<=70 AND roteirizado!='sim' and ativo=0 group by codigo_cliente";
...
if ($pega_listagem == "blue") {
  ...
  if($pega_escolha==""){   
    $query = "select codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, premium, ref from clientes where confianca_cad=101 AND roteirizado!='sim' and ativo=0 group by codigo_cliente";
    } else {
...
if ($pega_listagem == "roxo") {
  ...
  if($pega_escolha==""){   
  $query = "select codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, premium, ref from clientes where roteirizado!='sim' and ativo=1 group by codigo_cliente";
  } else {
...
<th nowrap="nowrap" headers="icone" bgcolor="#00000" style="color:#000000;" class="drag-enable" title="REFERÊNCIA">
    ...
    <td align="center" nowrap="nowrap" width="auto" style="width: 20px;text-align: center;"><input type="checkbox" class="marcar" name="check_list[]" id="check_list" value="<?php echo $row["codigo_cliente"]; ?>"></td>
      ...
      ?>
       ...
       // echo $row["cep"] 
        ...
        <td align="center" nowrap="nowrap" title='<?php echo $row["ref"]; ?>'><?php 
        //echo $row["premium"] 
        echo mb_strimwidth($row["ref"], 0, 20, "...");
        ?></td>
