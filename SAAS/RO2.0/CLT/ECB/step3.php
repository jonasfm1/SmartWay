...
if(!empty($_SESSION['city']) && count($_SESSION['city']) ){

	$campo0 = $_SESSION["city"];
	$string0 = '"' . implode('","', $campo0) . '"';
...
if($string0==""){
    ...
    $completa1 = " AND cidade in($string0) ";
}
...
//////////////CONTROLE DE LAYERS ////////////////////////
  ...
  layers_fixo_restricao = new google.maps.KmlLayer({
   		 url: 'http://www.caddsaas.com.br/SAAS/RO2.0/KMZ/ecb_restricao1.kmz',
    	 map: map, preserveViewport: true 
  		 });
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0" align="left" style=padding-bottom: 8px; width:500px" >
		<tr>
			...
			if($veiculo1!=''){
				...
				$query = "select codigo, endereco, cidade, estado, cep, codigo_cliente, confianca_cad, latitude_cad, longitude_cad, endereco_cad, nome, veiculo, peso, volume, valor, codigo_pedido, obs_pedido, roteirizado, data_pedido, premium, data_agendado, obs_agendado, volume_l from clientes where roteirizado!='sim' $completa0 $completa1 $completa2  order by veiculo DESC, codigo_cliente ASC";																
				$rs = mysql_query($query);
				$totalderegistros = mysql_num_rows($rs);
...
<tr style="height: 30px;">
			...
			<td align="left" valign="middle" nowrap="nowrap" style="color:#000000;padding-bottom: 10px;">
			<span  nowrap="nowrap">
		<a href="javascript:local_novo();">
<i class="material-icons" style="font-size:18px; vertical-align:bottom;">add_location_alt</i><font size="1" style="text-align: left"><b>&nbsp;ALTERAR LOCAL DE ENTREGA</b></font>
</a>
</span>
			</td>
...
<th  nowrap="nowrap" headers="icone" bgcolor="#00000" style="color:#000000; width:10px" class="drag-enable" title="LOCALIZAÇÃO NOVA">
<div align="center" nowrap='nowrap' >L</div>
</th> 
...
<div align="center" nowrap='nowrap'>C</div>
</th>
...
<th  nowrap="nowrap" headers="icone" bgcolor="#00000" style="color:#000000; width:10px" class="drag-enable" title="DATA DO AGENDAMENTO"> 
...
<input type="checkbox" id="loc_nova" name="loc_nova[]" value="<?php echo $row["codigo_pedido"];?>"/>
</td>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0"  style="padding:0px">
			<tbody>
				...
				<td width="20%" align="right" bgcolor="#589bd4" style="text-align: center; font-weight: bold;"><span class="material-icons" style="font-size: 16px;color:white">near_me</span></td>
					<td width="80%" align="left" bgcolor="#589bd4" style="text-align: left; font-weight: bold; color: white;">CIDADE</td>
				</tr>
			</tbody>
		</table>	
	</div>
	...
	<div class="content" id='legendas_layer3'>
			...
			<form method="POST" name="aaa" id="aaa">
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0" style="padding-left: 5px;">
						<tbody> 
							<?php
								$query_city = "select * from cidade order by cidade ASC";																
								$rs3_city = mysql_query($query_city);	
								$i=0;							
								while($row3_city = mysql_fetch_array($rs3_city)){

								$result_city[] = $row3_city["cidade"];
								//$cor_setor[] = $row3_setor["color"];

								$query_n_city = "select * from clientes where cidade='$result_city[$i]' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2;																
								$rs_n_city = mysql_query($query_n_city);	
								$cont_city = mysql_num_rows($rs_n_city);
								...
								<td align="right" style="text-align: center;"><input type="checkbox" id="<?php echo $result_city[$i];?>" name="city[]" value="<?php echo $result_city[$i];?>"/></td>
								<td colspan="2" valign="middle" style="text-align: left;padding-left: 5px;" title="<?php echo $result_city[$i];?>"><?php 
							//	echo $result[$i] . '(' . $cont . ')';
								echo mb_strimwidth($result_city[$i], 0, 20, "...") . '<b>(' . $cont_city . ')</b>';
								?></td>
