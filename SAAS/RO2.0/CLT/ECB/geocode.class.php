<?php
  ...
  class geocode {
    ...
    public function __construct ($address, $city, $estado, $cep, $country) {
      $_URL = "https://maps.googleapis.com/maps/api/geocode/xml?sensor=false&language=pt-BR&key=" . $_SESSION['chave'] . "&address=";

      $url = $_URL . urlencode($address) . "," . urlencode($city) . "-" . urlencode($estado) . "," . urlencode($cep) . "," . urlencode($country);
