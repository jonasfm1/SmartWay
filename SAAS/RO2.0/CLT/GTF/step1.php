...
$arquivobackup_comdata = date("d-m-Y H-i-s") . "_" . $logado . "_" . $str;


$abreviado =  substr_replace($str,(strlen($str) > $max ? '...xlsb' : ''), $max);


$link = __DIR__ .'\\uploads\\'.$arquivobackup_comdata;

copy($_FILES['filename']['tmp_name'], $link);
...
$deleterecords2 = "DELETE FROM vendedor"; //Esvaziar a tabela classificacao
mysql_query($deleterecords2);

$deleterecords3 = "DELETE FROM cidade"; //Esvaziar a tabela
...
for ($row=2; $row<= $highestRow; $row++){
 ...
 // $valor = str_replace(",", ".", $valor);	//valor
  ...
  if ($peso>5500){
   ...
   if ($peso<=5500 AND $peso>2500){
      $premium = 'B';    
      }

   if ($peso<=2500 AND $peso>1200){
      $premium = 'C';
         
   }

   if ($peso<=1200 AND $peso>600){
   $premium = 'I';
      
   }
   if ($peso<=600 AND $peso>250){
   $premium = 'S';
...
if($escolha=="adicionar"){
   $query_confere = "select codigo_pedido, peso, valor, cod_produto from clientes where codigo_pedido='$pedido' and peso='$peso' and valor='$valor' and cod_produto='$cod_produto'";														
...
$dados = mysql_fetch_array($rs);
   $import="INSERT into clientes(codigo_cliente, nome, cidade, estado, cep, bairro, peso, volume, valor, premium, loja, cod_vendedor, vendedor, codigo_pedido, data_pedido, liberado, obs_pedido, data_carrega, data_entrega, cod_produto, produto, cod_cancao, classificacao)values('$cod_cliente', '$nome_cliente', '$cidade', '$uf', '$cep', '$bairro', '$peso', '$volume', '0', '$premium', '$loja', '$cod_vendedor', '$vendedor', '$pedido', '$data_pedido', '$liberado', '$obs_pedido', '$data_carrega', '$data_entrega', '$cod_produto', '$produto', '$cod_cancao', '$data_carrega')";	
...
$query_vendedor = "select vendedor from clientes group by vendedor";														
$rs_vendedor = mysql_query($query_vendedor);

while ($row_vendedor = mysql_fetch_array($rs_vendedor)) {
   $ve =  $row_vendedor['vendedor'];

//echo $row_classifica['classificacao'];

$query_ve = "INSERT into vendedor(vendedor) values ('$ve')";
$query_ve= mysql_query($query_ve);

}

$query_city = "select cidade from clientes group by cidade";														
$rs_city = mysql_query($query_city);

while ($row_city = mysql_fetch_array($rs_city)) {
   $ve_city =  $row_city['cidade'];


//echo $row_classifica['classificacao'];

$query_ve_city = "INSERT into cidade(cidade) values ('$ve_city')";
$query_ve_city= mysql_query($query_ve_city);
...
<td><p style="font-size:12px">NÚMEROS DE REGISTROS:</p></td>
...
<td><p style="font-size:12px">NÚMEROS DE PEDIDOS JÁ INSERIDOS NA BASE:</p></td>
<td><strong style="color:green">&nbsp;<?php echo $conta_registros_iguais; ?></strong></td>
</tr>
<tr style="height: 30px;">
...
<td><p style="font-size:12px">&nbsp;&nbsp;INSERIR LISTA!</p></td>
</tr>
