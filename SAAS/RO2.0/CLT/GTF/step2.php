...
if ($pega_listagem == "red" or $pega_listagem == "") {
  ...
  if($pega_escolha==""){   
    $query = "select codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, cod_vendedor, premium from clientes where confianca_cad<=50 AND roteirizado!='sim' AND ativo=0 group by codigo_cliente";
...
if ($pega_listagem == "orange") {
  ...
  if($pega_escolha==""){   
    $query = "select codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, cod_vendedor, premium from clientes where confianca_cad>50 AND confianca_cad<=70 AND roteirizado!='sim' and ativo=0 group by codigo_cliente";
...
if ($pega_listagem == "green") {
  ...
  if($pega_escolha==""){   
    $query = "select codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, cod_vendedor, premium from clientes where confianca_cad>70 AND confianca_cad<=100 AND roteirizado!='sim' and ativo=0 group by codigo_cliente";
...
if ($pega_listagem == "blue") {
  ...
  if($pega_escolha==""){   
    $query = "select codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, cod_vendedor, premium from clientes where confianca_cad=101 AND roteirizado!='sim' and ativo=0 group by codigo_cliente";
    } else {
...
if ($pega_listagem == "roxo") {
  ...
  if($pega_escolha==""){   
  $query = "select codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, cod_vendedor, premium from clientes where roteirizado!='sim' and ativo=1 group by codigo_cliente";
  } else {
...
<th nowrap="nowrap" headers="icone" bgcolor="#00000" style="color:#000000;" class="drag-enable" title="VENDEDOR">
<div align="center" >VEND.</div>
</th>
    ...
    <td align="center" nowrap="nowrap" width="auto" style="width: 20px;text-align: center;"><input type="checkbox" class="marcar" name="check_list[]" id="check_list" value="<?php echo $row["codigo_cliente"]; ?>"></td>
      ...
      ?>
       ...
       // echo $row["endereco"];
        ...
        ?></td>
        <td align="left" id="bairro" nowrap="nowrap" title="<?php echo $row['bairro'];?>"  ><?php 
       // echo $row["bairro"]; 
       ...
       // echo $row["cep"] 
        ...
        //echo $row["obs_pedido"] 
        echo mb_strimwidth($row["obs_pedido"], 0, 9, "...");
        ?></td>
        <td align="center" nowrap="nowrap" title='<?php echo $row['setor']; ?>'><?php 
        //echo $row["setor"] 
        echo mb_strimwidth($row["setor"], 0, 9, "...");
        ?></td>
        <td align="center" title='<?php echo $row['cod_vendedor']; ?>'><?php 
        //echo $row["tempo_servico"] 
        echo mb_strimwidth($row["cod_vendedor"], 0, 9, "...");
        ?> </td>
