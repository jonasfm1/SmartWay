										...
										//	var distancia =1;
											////////////////////////////
                                                $.post("proc_alt_ordem.php", ordem_atual, function (retorno) {
													...
													window.location="altera_ordem_g.php?rota=<?php echo $nome_rota?>&saida="+w+"&entrada="+x;
...
?>
           <div id="linha_table_01_x">
			 ...
			 <table border=0.5 cellpadding="0" cellspacing="0" class="tabela" height="25px" style="  border-color: black;background-color: #589bd4;color: white;font-weight: bold;"  width="2212px">
                <tr>
...
?>
    ...
    <li id="arrayordem_<?php echo $row['id']; ?>">
                ...
                <table border=0.5 cellpadding="0" cellspacing="0" class="tabela" width="2212px">
                <tr>
                   ...
                   <td width="299px" nowrap class="td"> <?php echo  substr_replace($row['nome_cliente'], (strlen($row['nome_cliente']) > $max ? '...' : ''), $max);  ?></td>
                    <td width="300px" nowrap class="td"> <?php echo substr_replace($row['endereco'], (strlen($row['endereco']) > $max ? '...' : ''), $max); ?></td>
                     <td width="249px" nowrap class="td"><?php echo substr_replace($row['bairro'], (strlen($row['bairro']) > $max1 ? '...' : ''), $max1); ?></td>
                      <td width="149px" nowrap class="td"><?php echo substr_replace($row['cidade'], (strlen($row['cidade']) > $max4 ? '...' : ''), $max4); ?></td>
                       <td width="149px" nowrap class="td"><?php echo substr_replace($row['codigo_pedido'], (strlen($row['codigo_pedido']) > $max4 ? '...' : ''), $max4); ?></td>
                       <td width="148px" nowrap class="td"><?php echo substr_replace($row['obs_pedido'], (strlen($row['obs_pedido']) > $max4 ? '...' : ''), $max4); ?></td>
                        <td width="70px"nowrap class="td"><?php echo substr_replace($row['tempo'], (strlen($row['tempo']) > $max ? '...' : ''), $max); ?></td>
                         <td width="38px" nowrap class="td"><?php echo substr_replace($row['tempo_servico'], (strlen($row['tempo_servico']) > $max6 ? '...' : ''), $max6); ?></td>
                          <td width="40px" nowrap class="td"><?php echo substr_replace($row['distancia'], (strlen($row['distancia']) > $max5 ? '' : ''), $max5); ?></td>
