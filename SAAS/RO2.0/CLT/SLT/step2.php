...
if ($pega_listagem == "orange") {
  ...
  if($pega_escolha==""){   
    $query = "select codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, premium, praca, desc_praca, filial, especial, lote from clientes where confianca_cad>50 AND confianca_cad<=70 AND roteirizado!='sim' and ativo=0 group by codigo_cliente";
...
if ($pega_listagem == "green") {
  ...
  if($pega_escolha==""){   
    $query = "select codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, premium, praca, desc_praca, filial, especial, lote from clientes where confianca_cad>70 AND confianca_cad<=100 AND roteirizado!='sim' and ativo=0 group by codigo_cliente";
...
if ($pega_listagem == "blue") {
  ...
  if($pega_escolha==""){   
    $query = "select codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, premium, praca, desc_praca, filial, especial, lote from clientes where confianca_cad=101 AND roteirizado!='sim' and ativo=0 group by codigo_cliente";
    } else {
...
if ($pega_listagem == "roxo") {
  ...
  if($pega_escolha==""){   
  $query = "select confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, premium, praca, desc_praca, filial, especial, lote from clientes where roteirizado!='sim' and ativo=1 group by codigo_cliente";
  } else {
...
<div align="center" >PRACA</div>
</th>
<th nowrap="nowrap" headers="icone" bgcolor="#00000" style="color:#000000;" class="drag-enable" title="VOLUME">
<div align="center" >DESC.PRACA</div>
</th>
<th nowrap="nowrap" headers="icone" bgcolor="#00000" style="color:#000000;" class="drag-enable" title="VOLUME">
    ...
    <td align="center" nowrap="nowrap" width="auto" style="width: 20px;text-align: center;"><input type="checkbox" class="marcar" name="check_list[]" id="check_list" value="<?php echo $row["codigo_cliente"]; ?>"></td>
      ...
      ?>
        ...
        ?></div>
         ...
         <td align="center" title="<?php echo $row['lote'];?>" nowrap="nowrap"  ><?php 
        echo $row["lote"];
       ...
       // echo $row["cep"] 
        ...
        ?></td> 
                <td align="center" nowrap="nowrap" title='<?php echo $row['praca']; ?>'><?php 
       // echo $row["desc_praca"] ;
        echo mb_strimwidth($row["praca"], 0, 9, "...");
        ?></td>
                <td align="center" nowrap="nowrap" title='<?php echo $row['desc_praca']; ?>'><?php 
       // echo $row["desc_praca"] ;
        echo mb_strimwidth($row["desc_praca"], 0, 9, "...");
        ?></td>
                <td align="center" nowrap="nowrap" title='<?php echo $row['canal']; ?>'><?php 
       // echo $row["canal"] ;
        echo mb_strimwidth($row["obs_pedido"], 0, 15, "...");
        ?></td>
        <td align="center" nowrap="nowrap" title='<?php echo $row['filial']; ?>'><?php 
        //echo $row["obs_pedido"] 
        echo mb_strimwidth($row["filial"], 0, 30, "...");
        ?></td>
        <td align="center" nowrap="nowrap" title='<?php echo $row['especial']; ?>'><?php 
        //echo $row["obs_pedido"] 
        echo mb_strimwidth($row["especial"], 0, 30, "...");
        ?></td>
