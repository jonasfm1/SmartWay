...
$query ="SELECT * FROM clientes WHERE roteirizado!='sim' AND ativo=0" . $completa0 . $completa1 . $completa2 . " order by codigo DESC";
...
//	$query_agrupar = "select * from clientes where codigo_cliente in(select B.codigo_cliente from clientes B group by B.codigo_cliente having count(*)>1)";
	$query_agrupar = "select *, COUNT(codigo_cliente) AS Qtd, SUM(peso) AS Qtd_peso, SUM(volume) AS Qtd_volume, SUM(valor) AS Qtd_valor from clientes where roteirizado!='sim' AND ativo=0 " . $completa0 . $completa1 . $completa2 . "group by codigo_cliente having count(codigo_cliente)>1";
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0" align="left" style="padding-top: 15px; padding-bottom: 8px; width:500px" >
		<tr>
			...
			if($veiculo1!=''){
				...
				$query = "select codigo, endereco, cidade, estado, cep, codigo_cliente, confianca_cad, latitude_cad, longitude_cad, endereco_cad, nome, veiculo, peso, volume, valor, codigo_pedido, obs_pedido, roteirizado, data_pedido, premium, data_agendado, obs_agendado from clientes WHERE veiculo='$id1' order by codigo_cliente";																
				$rs = mysql_query($query);
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tbody> 
							<?php
								...
								$query_n = "select * from clientes where classificacao='$result[$i]' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2;																
...
<div class="container" id="makeMeDraggable3">
	...
	<div class="content" id='legendas_layer3'>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0" style="padding-left: 5px;">
						<tbody> 
							<?php
								...
								$query_n_vendedor = "select * from clientes where setor_grupo='$result_vendedor[$i]' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2;																
		...
		<form method ="POST" action="scripts.php?acao=limpa_filtro_step3_vendedor" id="qqq">	
...
<div class="container" id="makeMeDraggable4">
	...
	<div class="content" id='legendas_layer3'>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0" style="padding-left: 5px;">
						<tbody> 
							<?php
								...
								$query_n_setor = "select * from clientes where setor='$result_setor[$i]' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2;																
		...
		<form method ="POST" action="scripts.php?acao=limpa_filtro_step3_setor" id="mmm">	
