...
// ARTERA LAT LONG CASO SEJA IGUAL(PEDIDOS NO MESMO CLIENTE)
...
//$nova_rota = 900000;
$query_id_veiculo = "SELECT DISTINCT veiculo from rotas order by prioridade ASC";
$rs_id_veiculo =  mysql_query($query_id_veiculo);

while ($dados2 = mysql_fetch_array($rs_id_veiculo)) {	

    $quem_e = $dados2['veiculo'];
    $rota_ok = $rota_ok + 1;

    $update_nomerota = "UPDATE rotas SET nome_rota='$rota_ok' WHERE veiculo = '$quem_e'";
    $rs_up = mysql_query($update_nomerota);

    //echo $rota_ok;
}

$update_numero_rota_volta = mysql_query("UPDATE numero_rota SET numero=$rota_ok");
...
if($num_rows1==0){
	?>
	<meta http-equiv="refresh" content="0; URL='step4_1_c.php?imgsel=<?php echo $qual_rota ?>&inicio=<?php echo $inicio ?>&final=<?php echo $final ?><?php echo $guarda_linha_check1; ?>'"/>
<?php
}
...
<input type="text" id="txtEnderecoPartida" name="txtEnderecoPartida" value="<?php echo $inicio ?>" />                   
<input type="text" id="txtEnderecoChegada" name="txtEnderecoChegada" value="<?php echo $final ?>"  /> 
</div>
</form>
...
<textarea name="inicio" rows="3" id="inicio"  ></textarea>
<textarea name="final" rows="3" id="final" ></textarea>
...
?>
<textarea name="check_list1[]" rows="3" id="check_list1"  hidden="hidden"><?php echo $guarda_linha_check1;?></textarea>
