...
if ($pega_listagem == "red" or $pega_listagem == "") {
  ...
  if($pega_escolha==""){   
    $query = "select codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, data_pedido, setor, latitude_cad, longitude_cad, status_p from clientes where confianca_cad<=50 AND roteirizado!='sim' AND ativo=0 group by codigo_cliente";
...
if ($pega_listagem == "orange") {
  ...
  if($pega_escolha==""){   
    $query = "select codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, data_pedido, setor, latitude_cad, longitude_cad, status_p from clientes where confianca_cad>50 AND confianca_cad<=70 AND roteirizado!='sim' and ativo=0 group by codigo_cliente";
...
if ($pega_listagem == "green") {
  ...
  if($pega_escolha==""){   
    $query = "select codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, data_pedido, setor, latitude_cad, longitude_cad, status_p from clientes where confianca_cad>70 AND confianca_cad<=100 AND roteirizado!='sim' and ativo=0 group by codigo_cliente";
...
if ($pega_listagem == "blue") {
  ...
  if($pega_escolha==""){   
    $query = "select codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, data_pedido, setor, latitude_cad, longitude_cad, status_p from clientes where confianca_cad=101 AND roteirizado!='sim' and ativo=0 group by codigo_cliente";
    } else {
...
if ($pega_listagem == "roxo") {
  ...
  if($pega_escolha==""){   
  $query = "select confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, data_pedido,setor, latitude_cad, longitude_cad, status_p from clientes where roteirizado!='sim' and ativo=1 group by codigo_cliente";
  } else {
...
?>
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0" align="left" style="padding-top: 25px; padding-bottom: 8px; width:100%" >
		...
		<table border="0" width="300px" cellpadding="0" cellspacing="0" style="text-align: left; padding-left:10px" class="tabela">
...
<th nowrap="nowrap" headers="icone" bgcolor="#00000" style="color:#000000; width:10px" class="drag-enable" title="STATUS">
...
<th nowrap="nowrap" headers="icone" bgcolor="#00000" style="color:#000000;" class="drag-enable" title="BAIRRO">
...
<th nowrap="nowrap" headers="icone" bgcolor="#00000" style="color:#000000;" class="drag-enable" title="OBS.">
...
<th nowrap="nowrap" headers="icone" bgcolor="#00000" style="color:#000000;" class="drag-enable" title="SETOR">
<div align="center" >SETOR</div>
</th>
...
while ($row = mysql_fetch_array($rs)) {
  ...
  $guarda = "h=" .  $lat[$conta] . "&v=" . $lgn[$conta] . "&reg=" . $row["codigo_cliente"] . "&endereco=" . $row["endereco"] . "&codigo=" . $row["codigo"] . "&cod_cli=" . $row["codigo_cliente"] . "&nome=" . $row["nome"] . "&cidade=" . $row["cidade"] . "&uf=" . $row["estado"] . "&cep=" . $row["cep"] . "&peso=" . $row["peso"] . "&valor=" . $row["valor"] . "&volume=" . $row["volume"];
...
?>
    ...
    <td align="center" nowrap="nowrap" width="auto" style="width: 20px;text-align: center;"><input type="checkbox" class="marcar" name="check_list[]" id="check_list" value="<?php echo $row["codigo_cliente"]; ?>"></td>
      ...
      <a href="javascript:geo('<?php echo $guarda;?>');" title="Localizar cliente" >
      <?php 
      if($row["latitude_cad"]=='' OR $row["longitude_cad"]==''){

      ?>
       <span class="material-icons" style="font-size: 14px;color:#FF0000;">location_on</span>
      <?php
...
?>
             ...
             <td align="center" style="background-color: <?php echo $cor; ?>;text-align: center;">
      
             <p style="color: <?php echo $cor; ?>;"><?php echo $cor_table; ?></p>
            </td>
      ...
      ?>
        ...
        <td align="center"><div align="center"><?php 
            $query_zz = "select codigo_cliente from clientes where codigo_cliente='$cod_cli[$conta]'";		
       ...
       // echo $row["endereco"];
        ...
        ?></td>
             <td align="left"  nowrap="nowrap" title="<?php echo $row['bairro'];?>" ><?php 
       // echo $row["endereco"];
        echo mb_strimwidth($row["bairro"], 0, 20, "..."); 
        ?></td>
        ...
        <td align="left" nowrap="nowrap" title='<?php echo $row['estado']; ?>'><?php
        //echo $row["cidade"] 
        echo mb_strimwidth($row["estado"], 0, 20, "..."); 
        ?> </td>
       ...
       // echo $row["cep"] 
        ...
        ?></td>
                ...
                <td align="center" nowrap="nowrap" title='<?php echo $row['obs_pedido']; ?>'><?php 
       // echo $row["canal"] ;
        echo mb_strimwidth($row["obs_pedido"], 0, 15, "...");
        ?></td>
      
      <td align="center" nowrap="nowrap" title='<?php echo $row['data_pedido']; ?>'><?php 
       // echo $row["canal"] ;
        echo mb_strimwidth($row["data_pedido"], 0, 15, "...");
        ?></td>

            
<td align="left" nowrap="nowrap" title='<?php echo $row['setor']; ?>'><?php 
       // echo $row["canal"] ;
        echo mb_strimwidth($row["setor"], 0, 15, "...");
        ?></td>
