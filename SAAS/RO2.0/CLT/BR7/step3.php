...
if($posx==0 AND $posy==0){
	...
	//MENU SUB-SETOR
	$posx_filtro_vendedor= 610;
	$posy_filtro_vendedor= 0;	

	//MENU SETOR
	$posx_filtro_setor= 410;
	$posy_filtro_setor= 0;	
...
$(document).ready(function(){
  var elementOffset = $('#makeMeDraggable3').offset({top:<?php echo $posy_filtro_vendedor ?>,left:<?php echo $posx_filtro_vendedor ?>});
});
...
$(document).ready(function(){
  var elementOffset = $('#makeMeDraggable_x').offset({top:<?php echo $posy_libera ?>,left:<?php echo $posx_libera ?>});
});
...
if(!empty($_POST['liberar'])){
	$_SESSION['liberar'] = $_POST['liberar'];
}
...
if(!empty($_SESSION['liberar']) && count($_SESSION['liberar']) ){

	$campo_l = $_SESSION["liberar"];
	$string_l = '"' . implode('","', $campo_l) . '"';
	//print_r($campo_l) ;
	//echo $string_l;
...
if($string_l==""){
	...
	if(preg_match('/outros/i', $string_l)){
		$string_l = $string_l . '," "';
    ...
    $completa_l = " AND status_p in($string_l) ";
	//echo $completa_l;
}
...
if(!empty($_SESSION['setor']) && count($_SESSION['setor']) ){

	$campo1 = $_SESSION["setor"];
...
if($string1==""){
	...
	if(preg_match('/outros/i', $string1)){
		$string1 = $string1 . '," "';
		
	}
    $completa2 = " AND setor in($string1) ";
}


// REGIAO ////////////////////////////////////////////////////////

if(!empty($_SESSION['vendedor']) && count($_SESSION['vendedor']) ){

	$campo5 = $_SESSION["vendedor"];
	$string5 = '"' . implode('","', $campo5) . '"';
...
if($string5==""){
	...
	if(preg_match('/outros/i', $string5)){
		$string5 = $string5 . '," "';
		
	}
    $completa1 = " AND regiao in($string5) ";
}
...
if($string3==""){
	...
	if(preg_match('/outros/i', $string3)){
		$string3 = $string3 . '," "';
		
	}
    $completa3 = " AND embarcador in($string3) ";
}

//////////////////////////////////

$query ="SELECT * FROM clientes WHERE roteirizado!='sim' AND ativo=0 " . $completa0 . $completa1 . $completa2 .  $completa3 . $completa_l . " order by codigo DESC";
//echo $query;
...
<div id="abre_agendado">

<a href="#" id="mostrar">
<table border="0" style="width:100%; height:35px;text-align:center">
<tr>
<td style="font-size:10px ;width:25%; color:#FFF">
<i class="material-icons" style="font-size:18px; color:#FFF">today</i>
</td>
<td style="font-size:10px ;width:75	%; color:#FFF">
<font size="1"><strong>&nbsp;AGENDADOS PARA HOJE</strong></font>
</td>
</tr>
</table>
</a>
...
<?php
$query1 = "select * from clientes where data_agendado!='0000-00-00' AND roteirizado!='sim' AND ativo=0" . $completa0 . $completa1 . $completa2 .  $completa3 . $completa_l;															
$rs1 = mysql_query($query1);
...
while($row = mysql_fetch_array($rs)){
	...
	$especial[$mrk_cnt] = $row["especial"];
	$classificacao[$mrk_cnt]=$row["classificacao"];
	$cep[$mrk_cnt]=$row["cep"];

	$data_mysql = explode('-', $row["data_agendado"]);

	$status_pedido[$mrk_cnt] = $row["status_p"];

	$filtroesp1[$mrk_cnt] = $row["filtroesp1"];

	$perigo[$mrk_cnt]= $row["perigo"];
...
$data_agendado[$mrk_cnt] = $data_formatada;
	...
	if($veiculo[$mrk_cnt]==NULL){
		$conta_orfans++;
		$_conta_faltando_peso+=$valor[$mrk_cnt];
	}
...
id_status_pedido = <?php echo json_encode($status_pedido) ?>;
filtro_esp1 = <?php echo json_encode($filtroesp1) ?>;
perigo = <?php echo json_encode($perigo) ?>;
...
especial_js = <?php echo json_encode($especial) ?>;
...
/* 
const triangleCoords = [ { lat: -23.578160, lng: -46.913486 }, { lat: -23.551254, lng: -46.859585 }, { lat: -23.513796, lng: -46.909023 }, { lat: -23.549995, lng: -46.962238 } ];
...
//	$query_agrupar = "select * from clientes where codigo_cliente in(select B.codigo_cliente from clientes B group by B.codigo_cliente having count(*)>1)";
	$query_agrupar = "select *, COUNT(codigo_cliente) AS Qtd, SUM(peso) AS Qtd_peso, SUM(volume) AS Qtd_volume, SUM(valor) AS Qtd_valor from clientes where roteirizado!='sim' AND ativo=0 " . $completa0 . $completa1 . $completa2 .  $completa3  . $completa_l . "group by codigo_cliente having count(codigo_cliente)>1";
...
while($row = mysql_fetch_array($query_pontos)){
	...
	$lng_cad_pontos[$ie] = $row["longitude"];
        ...
        // ICONE ESPECIAL DA SEDE
                if ($nome_cad_pontos[$ie]=='SEDE BR7'){
...
for ($i = 0; $i < $mrk_cnt; $i++){
		...
		// SE FOR PRETO O ICONE LETRA BRANCA/////////////////////////////////////////////////////////////////////////////////////
			if($premium[$i]=='000000'){
				$icone_google ="https://chart.googleapis.com/chart?chst=d_map_pin_letter&chld=". $somaletra ."|$premium[$i]|FFF";
			} else {
				$icone_google ="https://chart.googleapis.com/chart?chst=d_map_pin_letter&chld=". $somaletra ."|$premium[$i]|000000";
			}
			if($perigo[$i]=='1'){


				if($premium[$i]=='000000'){
					$icone_google = "https://chart.googleapis.com/chart?chst=d_map_xpin_letter&chld=pin_star|D|$premium[$i]|FFF|FFA500";
				} else {
					$icone_google = "https://chart.googleapis.com/chart?chst=d_map_xpin_letter&chld=pin_star|D|$premium[$i]|000000|FFA500";
				}
	  ...
	  if($filtroesp1[$i]=='A'){
		...
		// SE FOR PRETO O ICONE LETRA BRANCA/////////////////////////////////////////////////////////////////////////////////////
			...
			if($perigo[$i]=='1'){
				...
				if($premium[$i]=='000000'){
					$icone_google = "https://chart.googleapis.com/chart?chst=d_map_xpin_letter&chld=pin_star|A|$premium[$i]|FFF|FFA500";
				} else {
					$icone_google = "https://chart.googleapis.com/chart?chst=d_map_xpin_letter&chld=pin_star|A|$premium[$i]|000000|FFA500";
				}
		...
		if($filtroesp1[$i]!='D' && $filtroesp1[$i]!='A'){

			

			if($premium[$i]=='000000'){
				$icone_google = "https://chart.googleapis.com/chart?chst=d_map_pin_letter&chld=|$premium[$i]|FFF";
			} else {
				$icone_google = "https://chart.googleapis.com/chart?chst=d_map_pin_letter&chld=|$premium[$i]|000000";
			}
			...
			if($perigo[$i]=='1'){
				...
				if($premium[$i]=='000000'){
					$icone_google = "https://chart.googleapis.com/chart?chst=d_map_xpin_letter&chld=pin_star||$premium[$i]|FFF|FFA500";
				} else {
					$icone_google = "https://chart.googleapis.com/chart?chst=d_map_xpin_letter&chld=pin_star||$premium[$i]|000000|FFA500";
				}
    	...
    	echo "mrktx$i = \"<div id='iw_container'><div class='iw_title' style='background-color:#589bd4'><table width='100%' border='0' cellspacing='0' cellpadding='0'><tr><td width='30px'><img src='imgs/ico_realizado.png' width='22' height='22'/></td><td style='text-wrap:normal;width:400'>$string_menor[$i] ($id_cliente[$i])</td></tr></table></div><table width='100%' border='0' cellspacing='0' cellpadding='0'><tr><td width='40px'></td><td style='padding-top:20px; padding-bottom:5px; font-size:11px'><strong>PEDIDO:</strong> $codigo_pedido[$i]</td></tr><tr><td width='40px'></td><td style='padding-top:5px; padding-bottom:5px; font-size:11px'><strong>PESO:</strong> $peso[$i] - <strong>VOLUME:</strong> $volume[$i] - <strong>VALOR:</strong> $valor[$i]</td></tr><tr><td width='40px'></td><td style='padding-top:5px; padding-bottom:5px; font-size:11px'><strong>END.:</strong>$end_menor[$i]</td></tr><tr><td width='40px'></td><td style='padding-top:5px; padding-bottom:5px; font-size:11px'><strong>BAIRRO/CIDADE: </strong>$bairro[$i], $cidade[$i]</td></tr><tr><td width='40px'></td><td style='padding-top:5px; padding-bottom:5px; font-size:11px' title='$obs_pedido[$i]'><strong>OBS.: </strong>$obs_pedido1[$i]</td></tr><tr><td width='40px'></td><td style='padding-top:5px; padding-bottom:5px; font-size:11px' title='$data_agendado[$i]'><strong>DATA AGENDADO: </strong>$data_agendado[$i]</td></tr><tr><td width='40px'></td><td style='padding-top:5px; padding-bottom:5px; font-size:11px' title='$obs_agendado[$i]'><strong>OBS.AGENDADO: </strong>$obs_agendado_title[$i]</td></tr><tr><td></td><td style='padding-top:5px; padding-bottom:5px; font-size:11px' title='$setor[$i]'><strong>SETOR: </strong>$setor[$i] - <strong>REGIÃO: </strong>$gsetor[$i]</td></tr><tr><td width='40px'></td><td style='padding-top:5px; padding-bottom:5px; font-size:11px'><a href='javascript:geo(id_cliente_js[$i])';> <i class='material-icons' style='font-size:20px; vertical-align:bottom'>edit_location_alt</i>&nbsp;LOCALIZAR&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a><a href='javascript:editar(codigo_pedido_js[$i])';><i class='material-icons' style='font-size:20px; vertical-align:bottom'>edit</i>&nbsp;EDITAR</a></td></tr></table></div>\";";
    	...
    	echo "marker[$i] = new google.maps.Marker({position: point$i, map: map,  label: {
			color: 'black',
			fontWeight: 'bold',
			text: '$classificacao[$i]',
			fontSize: '10px',
			
			className: 'marker-position',
		  }, title: '$nome[$i]($codigo_pedido[$i]) • PESO: $peso[$i] • VOL.: $volume[$i]• VALOR: $valor[$i] • OBS.: $obs_pedido[$i] • DATA INCLUSÃO: $classificacao[$i] • CEP: $cep[$i]', 
		  icon: '$icone_google',});\n";
  ...
  function ShowMarkers() {
	...
	//alert(numero_resgate);
        ...
        for (var i = 0; i < marker.length; i++) {
			...
			guarda_status_p[conta] = id_status_pedido[i];
			guarda_filtroesp1[conta] = filtro_esp1[i];
				...
				especial[conta] = especial_js[i];

				guarda_perigo[conta] = perigo[i];
...
//////////////////////////////////////////////////////////////////////////////////// 
//////////////////////////////////////////////////////////////////////////////////// 
if (newShape.Contains(point) && (guarda_status_p[i]=='4' || guarda_status_p[i]=='5')) {
...
//////////////////////////////////////////////////////////////////////////////////// 
					...
					if(premium[i]!='' || premium[i]!="0"){
						...
						//alert(guarda_perigo[i]);

						if(guarda_filtroesp1[i] == 'A'){
							somaletra = 'A';
							
							if(premium[i]=='000000'){
								linkado = "https://chart.googleapis.com/chart?chst=d_map_pin_letter&chld=" + somaletra + "|" + premium[i] + "|FFF";
							} else {
								linkado = "https://chart.googleapis.com/chart?chst=d_map_pin_letter&chld=" + somaletra + "|" + premium[i] + "|000000";

							}

							if(guarda_perigo[i]=='1'){

								if(premium[i]=='000000'){
								linkado = "https://chart.googleapis.com/chart?chst=d_map_xpin_letter&chld=pin_star|" + somaletra + "|" + premium[i] + "|FFF|FFA500";
									} else {
								linkado = "https://chart.googleapis.com/chart?chst=d_map_xpin_letter&chld=pin_star|" + somaletra + "|" + premium[i] + "|000000|FFA500";
							...
							linkado = "https://chart.googleapis.com/chart?chst=d_map_pin_letter&chld=|" + premium[i] + "|000000";

							if(guarda_perigo[i]=='1'){

								if(premium[i]=='000000'){
								linkado = "https://chart.googleapis.com/chart?chst=d_map_xpin_letter&chld=pin_star||" + premium[i] + "|FFF|FFA500";
								} else {
								linkado = "https://chart.googleapis.com/chart?chst=d_map_xpin_letter&chld=pin_star||" + premium[i] + "|000000|FFA500";
			...
			fontSize: '10px',
			className: 'marker-position',
		  },title: junta_title, icon: linkado,});	
					} else {							
					marker[i] = new google.maps.Marker({position: point, map: map, label: {
			color: 'black',
			fontWeight: 'bold',
			...
			fontSize: '10px',
			className: 'marker-position',
		  },title: junta_title, icon: "imgs/pickup_camper_pin.png"});	
	...
	//	alert(total_peso);
		...
		if(total_peso==0 && total_volume==0 && total_valor==0){
			//alert(total_peso);
			apDiv17_orange.style.visibility= 'hidden';
...
<div id="mySidenav" class="sidenav">
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0" align="left" style="padding-top: 15px; padding-bottom: 8px; width:500px" >
		<tr>
			...
			if($veiculo1!=''){
				...
				$query = "select codigo, endereco, cidade, estado, cep, codigo_cliente, confianca_cad, latitude_cad, longitude_cad, endereco_cad, nome, veiculo, peso, volume, valor, codigo_pedido, obs_pedido, roteirizado, data_pedido, premium, data_agendado, obs_agendado, status_p, perigo from clientes WHERE veiculo='$id1' $completa0 $completa1 $completa2 $completa3  $completa_l  order by codigo_cliente";																
				$rs = mysql_query($query);
				...
				$query = "select codigo, endereco, cidade, estado, cep, codigo_cliente, confianca_cad, latitude_cad, longitude_cad, endereco_cad, nome, veiculo, peso, volume, valor, codigo_pedido, obs_pedido, roteirizado, data_pedido, premium, data_agendado, obs_agendado, status_p, perigo from clientes where roteirizado!='sim' $completa0 $completa1 $completa2 $completa3 $completa_l order by veiculo DESC, codigo_cliente ASC";																
				$rs = mysql_query($query);
				$totalderegistros = mysql_num_rows($rs);
...
<thead>
<tr  height="35px" width="850px" >  
 
<th  nowrap="nowrap" headers="icone" bgcolor="#00000" style="color:#000000; width:10px" class="drag-enable" title="AGENDADO">
<div align="center" nowrap='nowrap' >A</div>
</th> 
<th  nowrap="nowrap" headers="icone" bgcolor="#00000" style="color:#000000; width:10px" class="drag-enable" title="AGENDADO">
<div align="center" nowrap='nowrap' >STS</div>
</th>  

<th nowrap="nowrap" headers="icone" bgcolor="#00000" style="color:#000000; width:10px" class="drag-enable" title="CÓDIGO PEDIDO"> 
<div align="center" nowrap='nowrap'>C.PEDIDO</div>
</th>                                                       
<th  nowrap="nowrap" headers="icone" bgcolor="#00000" style="color:#000000; width:10px" class="drag-enable" title="CÓDIGO CLIENTE"> 
...
<th nowrap="nowrap" headers="icone" bgcolor="#00000" style="color:#000000; width:10px" class="drag-enable" title="VOLUME"> 
<div align="center" nowrap='nowrap'>VO</div>
</th>
...
<th  nowrap="nowrap" headers="icone" bgcolor="#00000" style="color:#000000; width:10px" class="drag-enable" title="DATA INCLUSÃO"> 
<div align="center" nowrap='nowrap'>DATA</div>
</th>
...
<th  nowrap="nowrap" headers="icone" bgcolor="#00000" style="color:#000000; width:10px" class="drag-enable" title="DATA DO AGENDADAMENTO"> 
<div align="center" nowrap='nowrap'>D.AG</div>
</th>
<th  nowrap="nowrap" headers="icone" bgcolor="#00000" style="color:#000000; width:10px" class="drag-enable" title="OBS AGENDAMENTO"> 
<div align="center" nowrap='nowrap'>OBS.AG.</div>
</th>
...
<td align="center" nowrap='nowrap' title="<?php echo $row["data_agendado"] ?>" >
<?php
//if($row["premium"]=='R'){
?>
<a href="javascript:editar(<?php echo $row["codigo_pedido"];?>)">
<i class="material-icons" style="font-size:14px; color:#000000">today</i>
</a>
<?php
//}
?>
...
<?php 
//echo $row["status_p"];
$cor_table = $row["status_p"];

if($cor_table==1){ //em transito faturado
	$cor = "#808080";
  }
  if($cor_table==2){ //no cd sem faturar
	$cor = "#0000FF";
  }
  if($cor_table==3){ //sem processar
	$cor = "#FFE135";
  }
  if($cor_table==4){ //processado
	$cor = "#00FF00";
  }
  if($cor_table==5){ //re-entrega
	$cor = "#FF0000";
  }
  if($cor_table==6){ //devoluçaõ industria
	$cor = "#000000";
  }
  //ERRO NO STATUS
  if($cor_table==0 or $cor_table==""){
	$cor = "#FF007F";
  }
?>

<td align="center" nowrap='nowrap' style="background-color: <?php echo $cor; ?>;"> 
<span style="color: <?php echo $cor; ?>;font-size: 1px"><?php echo $cor_table; ?></span>

<?php
$perigo = $row["perigo"];
if($perigo=='1'){
?>
<span class="material-icons" style="font-size: 10px;color:#FFA500;text-align:center;">star</span>
<?php	
}
?>
...
<td align="center" nowrap='nowrap' title="<?php echo $row["data_pedido"] ?>" >
...
<td align="center" nowrap='nowrap'  >
<?php
if($row["data_agendado"]!='0000-00-00'){

	?>
	<i class="material-icons" style="font-size:14px; color:#000000">date_range</i>
<?php
} else {
?>
...
if($row["data_agendado"]!='0000-00-00'){
	...
	$data_br = implode('/', array_reverse(explode('-', $row["data_agendado"])));
	$hoje = date('d/m/Y');

	$data_br_compara = $row["data_agendado"];
	$hoje_compara = date('Y/m/d');
	
	if(strtotime($hoje_compara) == strtotime($data_br_compara)){

?>
<td align="center" nowrap='nowrap' style="background-color: #008000;">
<?php
	echo "<span style='color:#FFFFFF'>" . $data_br . "</span>";
?>
</td>
<?php

} elseif(strtotime($hoje_compara) > strtotime($data_br_compara)){
?>
<td align="center" nowrap='nowrap'  style="background-color: red;">
<?php
echo "<span style='color:#FFFFFF'>" . $data_br . "</span>";
?>
</td>
<?php
	...
	?>
	<td align="center" nowrap='nowrap'  style="background-color: #FF00FF;">
	<?php
echo "<span style='color:#FFFFFF'>" . $data_br . "</span>";
	?>
	</td>
...
<td align="center" nowrap='nowrap' title="<?php echo $row["obs_agendado"] ?>" >
<?php
if($row["data_agendado"]!='0000-00-00'){

	echo mb_strimwidth($row["obs_agendado"], 0, 15, "..."); 
...
<div class="container" id="makeMeDraggable2">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tbody>
				<tr>
					...
					<td width="85%" align="left" bgcolor="#589bd4" style="text-align: left; font-weight: bold; color: white;">&nbsp;DATA CTE</td>
	...
	<div class="content" id='legendas_layer2'>
			...
			<form action="" method="POST" name="yyy" id="yyy">
				...
				<div id='layers1'>
					<table width="250px" border="1" cellspacing="0" cellpadding="0"  class='bordasimples' style="margin-right: 5px;">
						<tbody> 
						<tr style="background-color: #ECECEC; height:30px">
								<td align="center" nowrap style="padding-left:5px; padding-right:5px"></td>
								<td align="center" nowrap style="padding-left:5px; padding-right:5px; width:50px" title="DATA CTE"><b>DATA</b></td>			
								<td align="center" nowrap style="padding-left:5px; padding-right:5px" title="PEDIDOS"><b>P</b></td>
								<td align="center" nowrap style="padding-left:5px; padding-right:5px" title="VISITAS"><b>V</b></td>
								<td align="center" nowrap style="padding-left:5px; padding-right:5px" title="PESO"><b>PESO</b></td>
						</tr>
							<?php
								...
								$query_n = "select *, SUM(peso) as peso_todo, COUNT(codigo) as total_registro from clientes where classificacao='$result[$i]' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2 . $completa3 . $completa_l;															
								$rs_n = mysql_query($query_n);	
							//	$cont = mysql_num_rows($rs_n);			
								
								$query_n1 = "select * from clientes where classificacao='$result[$i]' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2 . $completa3 . $completa_l . " group by codigo_cliente";															
								$rs_n1 = mysql_query($query_n1);	
								$contx = mysql_num_rows($rs_n1);

								$dados1 = mysql_fetch_array($rs_n);
								$pesototal1= $dados1['peso_todo'];

								$cont = $dados1['total_registro'];
								...
								<td align="center" style="text-align: center; width:12%"><input type="checkbox" id="<?php echo $result[$i];?>" name="classificacao[]" value="<?php echo $result[$i];?>"/></td>
								<td  valign="middle" style="text-align: left" title="<?php echo $result[$i];?>">
								<?php 
								echo mb_strimwidth($result[$i], 0, 10, "...");
								?>
								</td>
								<td  valign="middle" style="text-align: center" title="<?php echo $cont;?>">
								<?php 
								echo '<b>' . $cont . '</b>';
								?>
								</td>
								<td  valign="middle" style="text-align: center" title="<?php echo $contx;?>">
								<?php 
								echo '<b>' . $contx . '</b>';
								?>
								</td>
								<td  valign="middle" style="text-align: center" title="<?php echo $pesototal1;?>">
								<?php 
								echo '<b>' . $pesototal1 . '</b>';
								?>
								</td>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0"  style="padding:0px">
			<tbody>
				...
				<td width="20%" align="right" bgcolor="#589bd4" style="text-align: center; font-weight: bold;"><span class="material-icons" style="font-size: 16px;color:white">group </span></td>
					<td width="80%" align="left" bgcolor="#589bd4" style="text-align: left; font-weight: bold; color: white;">SETOR</td>
				</tr>
			</tbody>
		</table>	
	</div>
			...
			<form method="POST" name="sss" id="sss">
				...
				<div id='layers1'>
					<table width="100%" border="0" cellspacing="0" cellpadding="0" style="padding-left: 5px;">
						<tbody> 
							<?php
								$query_setor = "select * from setor order by setor ASC";																
								$rs3_setor = mysql_query($query_setor);	
								$i=0;							
								while($row3_setor = mysql_fetch_array($rs3_setor)){

								$result_setor[] = $row3_setor["setor"];
								//$cor_setor[] = $row3_setor["color"];

								$query_n_setor = "select * from clientes where setor='$result_setor[$i]' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2 .  $completa3 .$completa_l ;																
								...
								if($cont_setor>0){
								?>
								<td align="right" style="text-align: center;"><input type="checkbox" id="<?php echo $result_setor[$i];?>" name="setor[]" value="<?php echo $result_setor[$i];?>"/></td>
...
<div class="container" id="makeMeDraggable_x">
	<div class="header">
		<table width="100%" border="0" cellspacing="0" cellpadding="0"  style="padding:0px">
			<tbody>
				<tr>
					<td align="right" bgcolor="#589bd4" style="text-align: left; font-weight: bold;" height="32">&nbsp;</td>
					<td width="10%" align="right" bgcolor="#589bd4" style="text-align: center; font-weight: bold;">
					<span class="material-icons" style="font-size: 16px;color:white">view_timeline </span>
				</td>
					<td width="90%" align="left" bgcolor="#589bd4" style="text-align: left; font-weight: bold; color: white;">STATUS</td>
	...
	<div class="content" id='legendas_layer2'>
		...
		<form method="POST" name="ccc" id="ccc">
			...
			<div id='layers1'>
				<table width="100%" border="0" cellspacing="0" cellpadding="0" style="padding-left: 5px;">
					...
					$query_processado = mysql_query("select codigo from clientes where status_p='4' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2.  $completa3 . $completa_l);
					$cont_processado = mysql_num_rows($query_processado);															
					
					$query_reentrega = mysql_query("select codigo from clientes where status_p='5' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2.  $completa3. $completa_l);
					$cont_reentrega = mysql_num_rows($query_reentrega);

					$query_sprocessar = mysql_query("select codigo from clientes where status_p='3' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2.  $completa3. $completa_l);
					$cont_sprocessar = mysql_num_rows($query_sprocessar);

					$query_emtransito = mysql_query("select codigo from clientes where status_p='1' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2.  $completa3. $completa_l);
					$cont_emtransito = mysql_num_rows($query_emtransito);

					$query_nocd = mysql_query("select codigo from clientes where status_p='2' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2.  $completa3. $completa_l);
					$cont_nocd = mysql_num_rows($query_nocd);


					?>
<?php  
if ($cont_processado>0){
	?>
	<tr>				
	<td align="right" style="text-align: center;">
	<input type="checkbox" id="liberar" name="liberar[]" value="4"/>
	</td>
	<td colspan="2" valign="middle" style="text-align: left;padding-left: 5px;" title="PROCESSADO">
	PROCESSADO <?php echo '<b>(' . $cont_processado . ')</b>'; ?>
	</td>
	</tr>
...
<tr>			
<td align="right" style="text-align: center;"><input type="checkbox" id="liberar1" name="liberar[]" value="5"/></td>
<td colspan="2" valign="middle" style="text-align: left;padding-left: 5px;" title="REENTREGA">
REENTREGA <?php echo '<b>(' . $cont_reentrega . ')</b>'; ?>
</td>
</tr>						
...
<?php  
if ($cont_sprocessar>0){
	?>	
							<tr>			
							<td align="right" style="text-align: center;"><input type="checkbox" id="liberar2" name="liberar[]" value="3"/></td>
							<td colspan="2" valign="middle" style="text-align: left;padding-left: 5px;" title="SEM PROCESSAR">
							SEM PROCESSAR <?php echo '<b>(' . $cont_sprocessar . ')</b>'; ?>
							</td>
							</tr>
							<?php
} else {
...
?>
<?php  
if ($cont_emtransito>0){
	?>	

							<tr>			
							<td align="right" style="text-align: center;"><input type="checkbox" id="liberar3" name="liberar[]" value="1"/></td>
							<td colspan="2" valign="middle" style="text-align: left;padding-left: 5px;" title="EM TRANSITO">
							EM TRANSITO <?php echo '<b>(' . $cont_emtransito . ')</b>'; ?>
							</td>
							</tr>
							<?php
} else {
...
if ($cont_nocd>0){
	?>	
							<tr>			
							<td align="right" style="text-align: center;"><input type="checkbox" id="liberar4" name="liberar[]" value="2"/></td>
							<td colspan="2" valign="middle" style="text-align: left;padding-left: 5px;" title="NO CD S/FATURAR">
							NO CD S/FATURAR <?php echo '<b>(' . $cont_nocd . ')</b>'; ?>
							</td>
							</tr>
							<?php
} else {
...
?>
		...
		<input name="filtrar" id="filtrar" type="submit" value="FILTRAR" form="ccc"/><br>
	
	<form method ="POST" action="scripts.php?acao=limpa_filtro_step3" id="ddd">	
	<input name="limpar" id="limpar" type="submit" value="LIMPAR" form="ddd" onclick="limpa();"/>
	</form>
...
<div class="container" id="makeMeDraggable3">
	<div class="header">
		<table width="100%" border="0" cellspacing="0" cellpadding="0"  style="padding:0px">
			<tbody>
				<tr>
					<td align="right" bgcolor="#589bd4" style="text-align: left; font-weight: bold;" height="32">&nbsp;</td>
					<td width="20%" align="right" bgcolor="#589bd4" style="text-align: center; font-weight: bold;">
					<span class="material-icons" style="font-size: 16px;color:white">group_add </span>
				</td>
					<td width="80%" align="left" bgcolor="#589bd4" style="text-align: left; font-weight: bold; color: white;">REGIÃO</td>
	...
	<div class="content" id='legendas_layer2'>
			...
			<form method="POST" name="www" id="www">
		
				<div id='layers1'>
					<table width="100%" border="0" cellspacing="0" cellpadding="0" style="padding-left: 5px;">
						<tbody> 
							<?php
								$query_vendedor = "select * from regiao order by regiao ASC";																
								$rs3_vendedor = mysql_query($query_vendedor);	
								$i=0;							
								while($row3_vendedor = mysql_fetch_array($rs3_vendedor)){

								$result_vendedor[] = $row3_vendedor["regiao"];

								$query_n_vendedor = "select * from clientes where regiao='$result_vendedor[$i]' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2 .  $completa3 . $completa_l ;																
								$rs_n_vendedor = mysql_query($query_n_vendedor);	
								$cont_vendedor = mysql_num_rows($rs_n_vendedor);			
								?>
								<tr>
								<?php
								if($cont_vendedor>0){
								...
								<td align="right" style="text-align: center;"><input type="checkbox" id="<?php echo $result_vendedor[$i];?>" name="vendedor[]" value="<?php echo $result_vendedor[$i];?>"/></td>
								<td colspan="2" valign="middle" style="text-align: left;padding-left: 5px;" title="<?php echo $result_vendedor[$i];?>"><?php 
							//	echo $result[$i] . '(' . $cont . ')';
								echo mb_strimwidth($result_vendedor[$i], 0, 20, "...") . '<b>(' . $cont_vendedor . ')</b>';
								?></td>
								<?php
								}
								?>
            ...
            <input name="filtrar" id="filtrar" type="submit" value="FILTRAR" form="www"/><br>
...
<div class="container" id="makeMeDraggable7">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0"  style="padding:0px">
			<tbody>
				...
				<td width="20%" align="right" bgcolor="#589bd4" style="text-align: center; font-weight: bold;"><span class="material-icons" style="font-size: 16px;color:white">today </span></td>
					<td width="80%" align="left" bgcolor="#589bd4" style="text-align: left; font-weight: bold; color: white;">EMBARCADOR</td>
				</tr>
			</tbody>
		</table>	
	</div>
	...
	<div class="content" id='legendas_layer3'>
			...
			<form method="POST" name="aaa" id="aaa">
				...
				<div id='layers_embarcador'>
					<table width="500px" border="1" cellspacing="0" cellpadding="0"  class="bordasimples" style="margin-right: 6px;">
						<tbody> 

						<tr style="background-color: #ECECEC; height:30px">
								<td align="center" nowrap style="padding-left:5px; padding-right:5px"></td>
								<td align="left" nowrap style="padding-left:5px; padding-right:5px"><b>EMBARCADOR</b></td>			
								<td align="center" nowrap style="padding-left:5px; padding-right:5px"><b>TOTAL</b></td>
								<td align="center" nowrap style="background-color: #00FF00;padding-left:5px; padding-right:5px"><b>PROCESSADO</b></td>
								<td align="center" nowrap style="background-color: #FF0000;padding-left:5px; padding-right:5px"><b>REENTREGA</b></td>
								<td align="center" nowrap style="background-color: #FFE135;padding-left:5px; padding-right:5px"><b>SEM PROCESSAR</b></td>
								<td align="center" nowrap style="background-color: #808080;padding-left:5px; padding-right:5px"><b>EM TRANSITO</b></td>
								<td align="center" nowrap style="background-color: #0000FF;padding-left:5px; padding-right:5px"><b>NO CD S/ FATURAR</b></td>
								<td align="center" nowrap style="padding-left:5px; padding-right:5px"><b>PESO</b></td>
								<td align="center" nowrap style="padding-left:5px; padding-right:5px"><b>VOL.</b></td>
								<td align="center" nowrap style="padding-left:5px; padding-right:5px"><b>VALOR</b></td>
						</tr>
							<?php
								$query_dia = "select * from embarcador order by id ASC";																
								...
								$dia[] = $row3_dia["embarcador"];
								
								$query_n_dia = "select *, COUNT(codigo) AS conta_registro, SUM(peso) as peso_total, SUM(volume) as volume_total, SUM(valor) as valor_total from clientes where embarcador='$dia[$i]' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2.  $completa3 . $completa_l;																
								$rs_n_dia = mysql_query($query_n_dia);	
								//$cont_dia = mysql_num_rows($rs_n_dia);
								

								$dados = mysql_fetch_array($rs_n_dia);
								$pesototal= $dados['peso_total'];
								$volumetotal= $dados['volume_total'];
								$valortotal= $dados['valor_total'];
								$cont_dia = $dados['conta_registro'];

								// EM TRANSITO
								$query_trans = "select status_p from clientes where status_p='1' and embarcador='$dia[$i]' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2.  $completa3;	
								$rs_trans = mysql_query($query_trans);	
								$cont_trans = mysql_num_rows($rs_trans);

								// NO CD SEM FATURAR
								$query_sfaturar = "select status_p from clientes where status_p='2' and embarcador='$dia[$i]' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2.  $completa3;	
								$rs_sfaturar = mysql_query($query_sfaturar);	
								$cont_sfaturar = mysql_num_rows($rs_sfaturar);
								
								
								// SEM PROCESSAR
								$query_sprocessar = "select status_p from clientes where status_p='3' and embarcador='$dia[$i]' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2.  $completa3;	
								$rs_sprocessar = mysql_query($query_sprocessar);	
								$cont_sprocessar = mysql_num_rows($rs_sprocessar);

								// PROCESSADO
								$query_processado = "select status_p from clientes where status_p='4' and embarcador='$dia[$i]' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2.  $completa3;	
								$rs_processado = mysql_query($query_processado);	
								$cont_processado = mysql_num_rows($rs_processado);

								
								// RE ENTREGA
								$query_reentrega = "select status_p from clientes where status_p='5' and embarcador='$dia[$i]' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2.  $completa3;	
								$rs_reentrega = mysql_query($query_reentrega);	
								$cont_reentrega = mysql_num_rows($rs_reentrega);

								// INDUSTRIA
								$query_ind = "select status_p from clientes where status_p='6' and embarcador='$dia[$i]' and roteirizado!='sim' and ativo=0" . $completa0 . $completa1 . $completa2.  $completa3;	
								$rs_ind = mysql_query($query_ind);	
								$cont_ind = mysql_num_rows($rs_ind);								
							...
							<td align="left" style="text-align: center;"><input type="checkbox" id="<?php echo $dia[$i];?>" name="dia[]" value="<?php echo $dia[$i];?>"/>
									
							</td>
							<td align="left" style="text-align: left;" nowrap title="<?php echo $dia[$i];?>">
							<?php
							echo mb_strimwidth($dia[$i], 0, 20, "...");
							?>
							
							<td valign="middle" style="text-align: center;padding-left: 5px;"  nowrap><?php 
							echo $cont_dia;
							?></td>
							<td valign="middle" style="text-align: center;padding-left: 5px;"  nowrap><?php 
							$resultado_processado = ($cont_processado / $cont_dia) * 100;
							if($cont_processado!=0){
							echo $cont_processado . " <b>(" . number_format($resultado_processado, 1, '.', '') .  " %)</b>";
							}
								
								?></td>
							<td valign="middle" style="text-align: center;padding-left: 5px;"  nowrap><?php 
							$resultado_reentrega = ($cont_reentrega / $cont_dia) * 100;
							if($cont_reentrega!=0){
							echo $cont_reentrega . " <b>(" . number_format($resultado_reentrega, 1, '.', '') .  " %)</b>";
							}
							?></td> 
							<td valign="middle" style="text-align: center;padding-left: 5px;"  nowrap><?php 
							$resultado_sprocessar = ($cont_sprocessar / $cont_dia) * 100;
							if($cont_sprocessar!=0){
							echo $cont_sprocessar . " <b>(" . number_format($resultado_sprocessar, 1, '.', '') .  " %)</b>";
							}
							?></td>
							<td valign="middle" style="text-align: center;padding-left: 5px;" nowrap><?php 
							$resultado_trans= ($cont_trans / $cont_dia) * 100;
							if($cont_trans!=0){
							echo $cont_trans . " <b>(" . number_format($resultado_trans, 1, '.', '') .  " %)</b>";
							}
							?></td>
							<td valign="middle" style="text-align: center;padding-left: 5px;" nowrap><?php 
							$resultado_sfaturar = ($cont_sfaturar / $cont_dia) * 100;
							if($cont_sfaturar!=0){
							echo $cont_sfaturar . " <b>(" . number_format($resultado_sfaturar, 1, '.', '') .  " %)</b>";
							}
							?></td>
            ...
            <input name="filtrar" id="filtrar_maior" type="submit" value="FILTRAR" form="aaa"/><br>
		
		<form method ="POST" action="scripts.php?acao=limpa_filtro_step3" id="qqq">	
		<input name="limpar" id="limpar_maior" type="submit" value="LIMPAR" form="qqq" onclick="limpa();"/>
		</form>
...
<div class ="container" id="makeMeDraggable" name="makeMeDraggable" onmouseout="return false"> 
	<div class="header">
		<table width="100%" border="0" cellspacing="0" cellpadding="0"  style="padding:0px; height:35px; width:80px">
			<tbody>
				<tr>
					<td width="100px" align="center" bgcolor="#589bd4" style="text-align: center; font-weight: bold; color: white;font-size:10px">LAYERS</td>
			</tr>
			</tbody>
		</table>	
	</div>  
    <div class="content" id='legendas_layer'>
		...
		<tr>
			<td align="right" style="text-align: center; width:12%"><input type="checkbox" id="<?php echo $pega_id; ?>" onclick="Applealert(<?php echo $row20['id']; ?>, document.yyy1.<?php echo $pega_id; ?>, 'http://www.caddsaas.com.br/SAAS/RO2.0/KMZ/<?php echo $pega_arquivo; ?>');"/></td>
			<td colspan="2" valign="middle" style="text-align: left" title="<?php echo $pega_nome;?>">&nbsp;<?php 
			//echo $pega_nome; 
			echo mb_strimwidth($pega_nome, 0, 25, "...");
			?></td>
			</tr>
...
<?php
  ...
  <tr bgcolor="#ECECEC" style="text-align: center;" height="34" >
    ...
    <td title="PEDIDOS RESTANTES"><font color="#000000" style="font-weight:bold;" >P.REST.</font></td>
    <td title="VALOR RESTANTE"><font color="#000000" style="font-weight:bold;">V. REST.</font></td>
  </tr>
  ...
  <tr bgcolor="#ECECEC" style="text-align: center; font-weight: bold; color:#000000" height="34" >
    <td title="PESO TOTAL">PESO T.</td>
    <td title="VOLUME TOTAL">VOLUME T.</td>
    <td title="VALOR TOTAL">VALOR T.</td>
  </tr>
...
<iframe name="pag2" src="" frameborder=0 id="pag2" scrolling="no" marginheight="0" marginwidth="0" trusted="yes"></iframe>
	...
	<div id="botao"  name="botao"><a href="# " onclick="document.getElementById('Paginab').style.display = 'none'" trusted="yes"><i class="material-icons" style="font-size:30px;color:black;">&#xe5c9;</i></a></div>
</div>
