...
function editar(campo, regiao) {

document.getElementById("editar").style.display = "block";

var teste = "step2_editar.php?codigo=" + campo + "&pega_lista=" + regiao;
//var campo = document.getElementById('pega_id_cliente');
//alert(teste);
// var form = document.getElementById("frm");
...
if ($pega_listagem == "orange") {
  ...
  if($pega_escolha==""){   
    $query = "select codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, data_pedido, setor, latitude_cad, longitude_cad, regiao_itabom from clientes where confianca_cad>50 AND confianca_cad<=70 AND roteirizado!='sim' and ativo=0 group by codigo_cliente";
...
if ($pega_listagem == "green") {
  ...
  if($pega_escolha==""){   
    $query = "select codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, data_pedido, setor, latitude_cad, longitude_cad, regiao_itabom from clientes where confianca_cad>70 AND confianca_cad<=100 AND roteirizado!='sim' and ativo=0 group by codigo_cliente";
...
if ($pega_listagem == "blue") {
  ...
  if($pega_escolha==""){   
    $query = "select codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, data_pedido, setor, latitude_cad, longitude_cad, regiao_itabom from clientes where confianca_cad=101 AND roteirizado!='sim' and ativo=0 group by codigo_cliente";
    } else {
...
if ($pega_listagem == "roxo") {
  ...
  if($pega_escolha==""){   
  $query = "select confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, data_pedido,setor, latitude_cad, longitude_cad, regiao_itabom from clientes where roteirizado!='sim' and ativo=1 group by codigo_cliente";
  } else {
...
<th nowrap="nowrap" headers="icone" bgcolor="#00000" style="color:#000000;" class="drag-enable" title="REGIAO">
<div align="center" >REGIÃO</div>
</th>
...
?>
    ...
    <td align="center" nowrap="nowrap" width="auto" style="width: 20px;text-align: center;"><input type="checkbox" class="marcar" name="check_list[]" id="check_list" value="<?php echo $row["codigo_cliente"]; ?>"></td>
      ...
      <td align="center" style="width: 20px;text-align: center;"><a href="javascript:editar(<?php echo "'" . $row["codigo_cliente"] . "'" ."," . "'" . $row["regiao_itabom"] . "'";?>)" title="Editar cliente" ><span class="material-icons" style="font-size: 14px;">edit</span></a></td>
...
<td align="left" nowrap="nowrap" title='<?php echo $row['regiao_itabom']; ?>'><?php 
       // echo $row["canal"] ;
        echo mb_strimwidth($row["regiao_itabom"], 0, 15, "...");
        ?></td>
