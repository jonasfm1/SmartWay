    ...
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
            <title>::. ROTAS ONLINE .:: CADD</title>
                ...
                <link rel="stylesheet" type="text/css" href="css/altera_ordem1.css">
	...
	$(function () {
		...
		//	var distancia =1;
				...
				$.post("proc_alt_ordem.php", ordem_atual, function (retorno) {
					...
					window.location="altera_ordem_novo2.php?rota=<?php echo $nome_rota?>&saida="+w+"&entrada="+x;
...
//  document.getElementById('distancia').value += route.legs[i].distance.value/1000;
// document.getElementById('temp').value += duracao;
<?php
//	$dist = "<script>document.write(distancia)</script>";
//	$temp = "<script>document.write(duracao)</script>";


//$query = "UPDATE rotas SET tempo='$temp', distancia='$dist' WHERE nome_rota='$rota'";
...
<div id="apDiv11">
	 <div id="div_recal"><input name="RECALCULAR ROTA" type="submit" id="submit2" title="RECALCULAR ROTA" value="RECALCULAR ROTA" 	onclick="document.getElementById('tempo_distancia').submit();"></div>
...
?>
            <div id="linha_table_01_x">
			 ...
			 <table border=0.5 cellpadding="0" cellspacing="0" class="tabela" height="25px" style="  border-color: black;background-color: #589bd4;color: white;font-weight: bold;">
                <tr>
                 <td width="77px" nowrap class="td">ROTA</td>
               <td width="29px" nowrap class="td">SEQ.</td>
      
             <td width="99px" nowrap class="td">VEICULO</td>
                 <td width="199px" nowrap class="td">TRANSP.</td>
                  <td width="98px" nowrap class="td">CODIGO</td>
                   <td width="299px" nowrap class="td">NOME</td>
                    <td width="300px" nowrap class="td">ENDEREÇO</td>
                     <td width="249px" nowrap class="td">BAIRRO</td>
                      <td width="149px" nowrap class="td">CIDADE</td>
                       <td width="149px" nowrap class="td">COD. PEDIDO</td>
                       <td width="148px" nowrap class="td">OBS. PEDIDO</td>
                        <td width="70px"nowrap class="td">TEMPO</td>
                         <td width="38px" nowrap class="td">T.S.</td>
                          <td width="40px" nowrap class="td">DIST.</td>
...
?>
    ...
    <li id="arrayordem_<?php echo $row['id']; ?>">
                ...
                <table border=0.5 cellpadding="0" cellspacing="0" class="tabela">
                <tr>
                 <td width="77px" nowrap class="td"><?php 
				 echo  substr_replace($row['nome_rota'], (strlen($row['nome_rota']) > $max2 ? '...' : ''), $max2);  
				 ?></td>
               <td width="29px" nowrap class="td"><?php echo $row['ordem']; ?></td>
...
?>
             ...
             <td width="99px" nowrap class="td"><?php echo $queme; ?></td>
                 <td width="199px" nowrap class="td"><?php echo $queme_transp; ?></td>
                  <td width="98px" nowrap class="td"><?php echo $row['codigo_cliente']; ?></td>
                   <td width="299px" nowrap class="td"> <?php echo  substr_replace($row['nome_cliente'], (strlen($row['nome_cliente']) > $max ? '...' : ''), $max);  ?></td>
                    <td width="300px" nowrap class="td"> <?php echo substr_replace($row['endereco'], (strlen($row['endereco']) > $max ? '...' : ''), $max); ?></td>
                     <td width="249px" nowrap class="td"><?php echo $row['bairro']; ?></td>
                      <td width="149px" nowrap class="td"><?php echo $row['cidade']; ?></td>
                       <td width="149px" nowrap class="td"><?php echo $row['codigo_pedido']; ?></td>
                       <td width="148px" nowrap class="td"><?php echo $row['obs_pedido']; ?></td>
                        <td width="70px"nowrap class="td"><?php echo $row['tempo']; ?></td>
                         <td width="38px" nowrap class="td"><?php echo $row['tempo_servico']; ?></td>
                          <td width="40px" nowrap class="td"><?php echo $row['distancia']; ?></td>
                           <td width="69px"nowrap class="td"><?php echo $row['peso']; ?></td>
                            <td width="69px"nowrap class="td"><?php echo $row['volume']; ?></td>  
                              <td width="69px" nowrap class="td"><?php echo $row['valor']; ?></td>
                               <td width="61px" nowrap class="td"><?php echo $row['setor']; ?></td>
...
<div id="directions-panel" hidden="hidden"></div>
