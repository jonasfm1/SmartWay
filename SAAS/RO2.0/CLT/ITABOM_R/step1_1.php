...
$query_iguais = "Select nome, codigo_cliente, latitude_cad, longitude_cad from clientes";
$rs_iguais= mysql_query($query_iguais);

$total_iguais = mysql_num_rows($rs_iguais);





while($row_iguais = mysql_fetch_array($rs_iguais)){

	$codigo_iguais = $row_iguais["codigo_cliente"];
	$nome_igual = $row_iguais["nome"];

	$ltd_igual = $row_iguais["latitude_cad"];
	$lgn_igual = $row_iguais["longitude_cad"];

	$concat_iguais = $ltd_igual . "," . "$lgn_igual";

	//echo $nome_igual;

	$update_iguais = "UPDATE clientes SET especial='$concat_iguais' where codigo_cliente='$codigo_iguais'";
	$rs_iguais2= mysql_query($update_iguais);
