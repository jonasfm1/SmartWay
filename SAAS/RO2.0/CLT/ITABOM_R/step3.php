...
//	$query_agrupar = "select * from clientes where codigo_cliente in(select B.codigo_cliente from clientes B group by B.codigo_cliente having count(*)>1)";
	$query_agrupar = "select *, COUNT(codigo_cliente) AS Qtd, SUM(peso) AS Qtd_peso, SUM(volume) AS Qtd_volume, SUM(valor) AS Qtd_valor from clientes where roteirizado!='sim' AND ativo=0 " . $completa0 . $completa1 . $completa2 . "group by especial having count(especial)>1";
...
while($row = mysql_fetch_array($query_pontos)){
	...
	$lng_cad_pontos[$ie] = $row["longitude"];
        ...
        // ICONE ESPECIAL DA SEDE
                if ($nome_cad_pontos[$ie]=='FABRICA RACAO - ITABOM'){
...
for ($i = 0; $i < $mrk_cnt; $i++){
    	...
    	echo "marker[$i] = new google.maps.Marker({position: point$i, map: map,  label: {
		  ...
		  }, title: '$nome[$i]($codigo_pedido[$i]) • PESO: $peso[$i] • VOL.: $volume[$i]• VALOR: $valor[$i] • OBS.: $obs_pedido[$i] • CLASSIFICAÇÃO: $classificacao[$i] • CEP: $cep[$i]', icon: '$icone_google',});\n";
    	...
    	echo "marker[$i] = new google.maps.Marker({position: point$i, map: map,  label: {
			...
			title: '$nome_veiculo - $nome[$i]($codigo_pedido[$i]) • PESO: $peso[$i] • VOL.: $volume[$i] • VALOR: $valor[$i] • OBS.: $obs_pedido[$i] • CLASSIFICAÇÃO: $classificacao[$i] • CEP: $cep[$i]', icon: 'imgs/$imagem_mapa',});\n";
		echo "google.maps.event.addListener(marker[$i],
         'click',  function() {
   		     infowindow$i.open(map,marker[$i]);
          });\n";
...
//////////////////////////////////////////////////////////////////////////////////// 
				...
				var junta_title = nome_js[i] + '(' + guarda_codigo_pedido[i] + ')' + ' • PESO: ' + guarda_peso[i] + ' • VOLUME: ' + guarda_volume[i] + ' • VALOR: ' + guarda_valor[i] + ' • OBS: ' + guarda_obs[i] + ' • CLASSIFICAÇÃO: ' + guarda_cla[i] + ' • CEP: ' + guarda_cep[i];
...
////////////////////////////////////////////////////////////////////////////////////	
						...
						for (var g = 0; g < id_tabela_veiculos.length; g++) {
							...
							var junta_title = nomes_veiculos[g] + '-' + nome_js[i] + '(' + guarda_codigo_pedido[i] + ')' + ' • PESO: ' + guarda_peso[i]  + ' • VOLUME: ' + guarda_volume[i] + ' • VALOR: ' + guarda_valor[i] + ' • OBS: ' + guarda_obs[i] + ' • CLASSIFICAÇÃO: ' + guarda_cla[i] + ' • CEP: ' + guarda_cep[i];
							marker[i] = new google.maps.Marker({position: point, map: map,  label: {
			color: 'black',
			fontWeight: 'bold',
			text: guarda_peso[i],
...
////////////////////////////////////////////////////////////////////////////////////	 
							...
							var junta_title = nomes_veiculos[s] + '-' + nome_js[i] + '(' + guarda_codigo_pedido[i] + ')' + ' • PESO: ' + guarda_peso[i] + ' • VOLUME: ' + guarda_volume[i] + ' • VALOR: ' + guarda_valor[i] + ' • OBS: ' + guarda_obs[i] + ' • CLASSIFICAÇÃO: ' + guarda_cla[i] + ' • CEP: ' + guarda_cep[i];
...
<a href="javascript:map.setCenter(new google.maps.LatLng(<?php echo $lat; ?>,<?php echo $lgn; ?>));map.setZoom(23);infobox(<?php echo $lat; ?>, <?php echo $lgn; ?>, '<?php echo 'Pedido:'. $cod_pedido . " - " . $nome_cliente; ?>');tempofora();"><i class="material-icons" style="font-size:14px">search</i></a>
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0" align="left" style=padding-bottom: 8px; width:500px" >
		<tr>
			...
			if($veiculo1!=''){
				...
				$query = "select codigo, endereco, cidade, estado, cep, codigo_cliente, confianca_cad, latitude_cad, longitude_cad, endereco_cad, nome, veiculo, peso, volume, valor, codigo_pedido, obs_pedido, roteirizado, data_pedido, premium, data_agendado, obs_agendado, volume_l from clientes where roteirizado!='sim' and ativo=0 $completa0 $completa1 $completa2  order by veiculo DESC, codigo_cliente ASC";																
				$rs = mysql_query($query);
				$totalderegistros = mysql_num_rows($rs);
...
<div align="center" nowrap='nowrap'>CIDADE</div>
</th>
...
<a href="javascript:map.setCenter(new google.maps.LatLng(<?php echo $lat; ?>,<?php echo $lgn; ?>));map.setZoom(23);infobox(<?php echo $lat; ?>, <?php echo $lgn; ?>, '<?php echo $nome_cliente_ei; ?>');tempofora();"><i class="material-icons" style="font-size:14px">search</i></a>
</td>
				...
				<div id='layers1'>
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tbody> 
							...
							//	echo $result[$i] . '(' . $cont . ')';
								echo mb_strimwidth($result[$i], 0, 24, "...") . '<b>(' . $cont . ')</b>';
								?></td>
								<?php
								}
								?>
