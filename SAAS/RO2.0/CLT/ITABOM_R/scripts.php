...
while ($row = mysql_fetch_array($seleciona)) {
    ...
    while ($row20 = mysql_fetch_array($rs20)) {
        ...
        $con2 = mysql_connect("localhost", "root", "HtMQZhAwCNzeaAfT") or die ("Sem conexão com o servidor");
...
<?php
			...
			if($redespacho==''){
				$query2 = "INSERT INTO rotas( data_teste, login, rota, sequencia, x, y, idcliente, nome, endereco, cidade, classificacao, tempo, raio, status, statusrota, lista, codigo_pedido, site, peso, volume, valor, obs, uf, cep) VALUES('$today_ok', '$login', '$rota', '$ordem', '$x', '$y', '$codigo_cliente', '$nome_cliente', '$endereco', '$cidade', '$tipo_veiculo ', '$tempo_servico', '$raio', '$status', $statusrota_ok, '$lista', '$codigo_pedido', 1, '$peso', '$volume', '$valor', '$obs_pedido', '$uf', '$cep')";
				
			} else {
				$query2 = "INSERT INTO rotas( data_teste, login, rota, sequencia, x, y, idcliente, nome, endereco, cidade, classificacao, tempo, raio, status, statusrota, lista, codigo_pedido, site, peso, volume, valor, obs, uf, cep) VALUES('$today_ok', '$redespacho', '$rota', '$ordem', '$x', '$y', '$codigo_cliente', '$nome_cliente', '$endereco', '$cidade', '$tipo_veiculo ', '$tempo_servico', '$raio', '$status', $statusrota_ok, '$lista', '$codigo_pedido', 1, '$peso', '$volume', '$valor', '$obs_pedido', '$uf', '$cep')";
...
/// MANIPULA A LONGITUDE DOS CLIENTES IGUAIS////////////////////////////////////////////////
	...
	$query_agrupar = "select *, COUNT(*) from clientes group by especial";
...
while($row_agrupar = mysql_fetch_array($query_agrupar)){
	...
	$esp = $row_agrupar['especial'];
	
	$query_agrupar1 = "select * from clientes where especial='$esp'";
