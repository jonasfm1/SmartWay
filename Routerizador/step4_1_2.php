...
function goBack() {
	...
	while($row_ok = mysql_fetch_array($query_somalgn)){
		...
		$lgn_muda = $row_ok1["longitude_cad"] . 0 . $cod;
		
		$lgn_muda1 = number_format($lgn_muda, 11, '.', '');
		
		
		$query_where10 = mysql_query("UPDATE clientes SET longitude_cad=$lgn_muda1 where codigo=$cod");
	...
	if(!empty($_GET['check_list1'])){
     foreach($_GET['check_list1'] as $report_id1){
		...
		while($row_fora = mysql_fetch_array($rs_fora)){
			...
			$query3_fora = "INSERT INTO rotas(codigo_cliente, nome_cliente, lat, lgn, veiculo, endereco, cidade, ordem, codigo_pedido, obs_pedido, peso, volume, valor, nome_rota) VALUES('$codigo_cliente_fora[$conta_fora1_new]', '$nome_cliente_fora[$conta_fora1_new]', '$lat_cliente_fora[$conta_fora1_new]', '$lgn_cliente_fora[$conta_fora1_new]', '$veiculo_cliente_fora[$conta_fora1_new]', '$endereco_cliente_fora[$conta_fora1_new]', '$cidade_cliente_fora[$conta_fora1_new]', '0', '$codigo_pedido_fora[$conta_fora1_new]', '$obs_pedido_fora[$conta_fora1_new]', '$peso_cliente_fora[$conta_fora1_new]', '$volume_cliente_fora[$conta_fora1_new]', '$valor_cliente_fora[$conta_fora1_new]', '$agrupamento_concatena')";
...
if(!empty($_GET['check_list'])){
     foreach($_GET['check_list'] as $report_id){
		...
		while($row = mysql_fetch_array($rs1)){
			...
			$query3 = "INSERT INTO rotas(codigo_cliente, nome_cliente, lat, lgn, veiculo, endereco, cidade, codigo_pedido, obs_pedido, peso, volume, valor) VALUES('$codigo_cliente[$conta1_new]', '$nome_cliente[$conta1_new]', '$lat_cliente[$conta1_new]', '$lgn_cliente[$conta1_new]', '$veiculo_cliente[$conta1_new]', '$endereco_cliente[$conta1_new]', '$cidade_cliente_fora[$conta1_new]', '$codigo_pedido[$conta1_new]', '$obs_pedido[$conta1_new]', '$peso_cliente[$conta1_new]', '$volume_cliente[$conta1_new]', '$valor_cliente[$conta1_new]')";
...
<div id="apDiv11"><img src="imgs/bg_box_step4_1.png" width="580" height="82" />
  ...
  <div id="help" class="help"><img src="imgs/help.png" width="45" height="45" alt=""/><div id="sidebar">
	<p>• <strong>ROTAS</strong> feitas!</p><br />
     <p>• Você pode ver os detalhes das Rotas no resultado a esquerda do mapa!</p><br />
    </div>
...
<div id="apDiv13_volta">
<form name="go_step4a" action="step4.php">
