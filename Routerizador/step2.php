...
if ($pega_listagem == "red" or $pega_listagem == "") {
  ...
  if($pega_escolha==""){   
    $query = "select classificacao, codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, premium from clientes where confianca_cad<=50 AND roteirizado!='sim' AND ativo=0 group by codigo_cliente";

    } else {
      $query = "select classificacao, codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, premium from clientes where confianca_cad<=50 AND roteirizado!='sim' AND ativo=0 group by codigo_cliente ORDER BY $pega_escolha $pega_ordem";
    }
...
if ($pega_listagem == "orange") {
  ...
  if($pega_escolha==""){   
    $query = "select classificacao, codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, premium from clientes where confianca_cad>50 AND confianca_cad<=70 AND roteirizado!='sim' and ativo=0 group by codigo_cliente";
...
if ($pega_listagem == "green") {
  ...
  if($pega_escolha==""){   
    $query = "select classificacao, codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, premium from clientes where confianca_cad>70 AND confianca_cad<=100 AND roteirizado!='sim' and ativo=0 group by codigo_cliente";
...
if ($pega_listagem == "blue") {
  ...
  if($pega_escolha==""){   
    $query = "select classificacao, codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, premium from clientes where confianca_cad=101 AND roteirizado!='sim' and ativo=0 group by codigo_cliente";
    } else {
      $query = "select classificacao, codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, premium from clientes where confianca_cad=101 AND roteirizado!='sim' and ativo=0 group by codigo_cliente ORDER BY $pega_escolha $pega_ordem";
    }
...
if ($pega_listagem == "roxo") {
  ...
  if($pega_escolha==""){   
  $query = "select classificacao, codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, premium from clientes where roteirizado!='sim' and ativo=1 group by codigo_cliente";
  } else {
    $query = "select classificacao, codigo, confianca_cad, endereco_cad, codigo_cliente, nome, endereco, bairro, cidade, estado, cep, SUM(peso) AS peso,  SUM(volume) AS volume,  SUM(valor) AS valor, obs_pedido, setor, premium from clientes where roteirizado!='sim' and ativo=1 group by codigo_cliente ORDER BY $pega_escolha $pega_ordem";
  }
    ...
    <td align="center" nowrap="nowrap" width="auto" style="width: 20px;text-align: center;"><input type="checkbox" class="marcar" name="check_list[]" id="check_list" value="<?php echo $row["codigo_cliente"]; ?>"></td>
      ...
      ?>
       ...
       // echo $row["cep"] 
        ...
        <td align="center" title='<?php echo $row['classificacao']; ?>'><?php 
        //echo $row["premium"] 
        echo mb_strimwidth($row["classificacao"], 0, 9, "...");
        ?></td>
